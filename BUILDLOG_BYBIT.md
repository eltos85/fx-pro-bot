...
**Контекст.** При обсуждении наблюдения «первые 1-2 сделки в день
почти всегда в плюс» вскрылся реальный механизм запаздывания:
главный цикл бота 5 минут, а средний hold-time скальп-сделки —
0.5–30 минут. Конкретно на серии 01.05 несколько SL'ов случились
**внутри одного 5-минутного окна** (открытие 20:05 → SL 20:05 за
30 секунд), бот физически не успел даже один раз посмотреть на
позицию через `EXIT-CHECK`. Trailing stop тоже мог пропустить пик
прибыли внутри окна.

**Что меняем.**
- `docker-compose.yml` (только секция `bybit-bot`):
  `BYBIT_BOT_POLL_INTERVAL_SEC` 300 → 60.
- `src/bybit_bot/app/main.py`: COF filter-funnel теперь печатается
  раз в `3600 // poll_interval_sec` циклов, а не раз в 12 циклов
  (которое означало «1ч при cycle=5m», на 1m стало бы 12 минут).
  Привязка к секундам, период остаётся ~1ч независимо от частоты.

**Что НЕ меняем.**
- Сигналы по-прежнему считаются на **5-минутных свечах** (yfinance
  interval = 5m, research-anchors Sword Red BTC / FMZQuant ETH —
  под 5m). Параметры стратегий не тронуты.
- SL/TP стоят на бирже, работают по тику — на это poll-интервал
  не влияет вообще (это объяснили в чате как защиту от убытков
  «реального времени»).
- Trailing stop — после установки тоже на бирже, работает по тику.

**Что улучшается.**
- **Установка trailing stop**: было до 5 мин запаздывание после
  достижения порога 0.7 ATR прибыли, стало до 1 мин.
- **Реакция на новый закрытый бар**: было среднее 2.5 мин, стало
  среднее 30 сек. Цена входа в mean-reversion сделку ближе к
  моменту закрытия сигнальной свечи.
- **EXIT-CHECK в логах** раз в минуту (раньше раз в 5 мин) —
  лучшая observability открытых позиций.
- **Точность time-stop 24ч**: ±1 мин (раньше ±5 мин). Несущественно
  по PnL, но аккуратнее.

**Нагрузка на API.** ~10 calls/cycle × 60s = **0.17 req/sec**.
Лимит Bybit retail: **10 req/sec**. Запас ×60. Никаких
rate-limit-проблем.

**Соответствие правилам.**
- `sample-size.mdc`: «технические улучшения / логирование, не
  влияют на торговлю» — допустимая правка без условий выборки.
- `no-data-fitting.mdc`: 60 секунд — это **минимальный шаг
  Bybit kline API** (1 минута), не подбор под результат. Естественная
  граница, не curve-fit.
- `strategy-guard.mdc`: торговая логика (пороги, фильтры, SL/TP
  множители, лимиты позиций) **не тронута**. Только частота опроса.

**Тесты.** Полный pytest suite — **350 passed, 0 failed**.

**Метрики для проверки T+14d (09.05).**
- Появились ли в логах новые EXIT-CHECK через 1 мин после
  открытия (раньше было невозможно — позиция закрывалась внутри окна)?
- Trailing stop устанавливается ли быстрее? (счётчик в логе, можно
  будет grep'ом измерить).
- Не появились ли API-ошибки rate-limit (`429 Too Many Requests`)?
- Изменился ли PnL pattern на whipsaw-сериях (если повторятся)?
  Главная гипотеза — точечно лучше за счёт trailing-stop, не общий
  win-rate.

**Файлы:** `docker-compose.yml`, `src/bybit_bot/app/main.py`.
...
**Период:** 27.04.2026 00:00 → 02.05.2026 03:30 UTC (5.1 торговых дней).
**Источник:** Bybit API `closedPnl` (NET, с комиссией) +
`/data/bybit_stats.sqlite` для разбивки по стратегиям.

**Сводка (n=19, все scalp_vwap LONG):**

| Метрика | Значение | Wave 6 baseline |
|---|---|---|
| NET PnL | **−$10.55** | — |
| Win Rate | 36.8% (7W/12L) | ≥55% ❌ |
| Avg Win | +$0.92 | — |
| Avg Loss | −$1.42 | — |
| Profit Factor | **0.38** | ≥1.0 (цель 1.2) ❌ |
| Expectancy/trade | −$0.55 | — |
| Avg hold | ~5 мин | — |
| n сделок | 19 | ≥100 (19%) |

**ORB London и COF дали 0 сделок** за весь период. ATR%/RSI/volume
условия не достигались — режим сжатия волатильности (см. запись
2026-04-29).

**По дням:**

| День | Сделок | NET | Комментарий |
|------|--------|-----|-------------|
| 27.04 Mon | 1 | −$4.52 | WIFUSDT (RR 0.64, до фикса RR 1:1.5) |
| 28.04 Tue | 1 | +$1.32 | ADAUSDT |
| 29.04 Wed | 1 | +$2.02 | TONUSDT |
| 30.04 Thu | 1 | +$2.20 | TONUSDT |
| **01.05 Fri** | **15** | **−$11.56** | **alt-selloff whipsaw** |
| 02.05 Sat | 0 | — | weekday-фильтр (не Sat) |

Первые 4 дня (27-30.04): **3W/1L, +$1.02 на 4 сделках** — Wave 6 работал
ровно как задумано. 01.05 — единичный плохой день съел всё.

**Эпизод 01.05 (Fri):**
- 15 сделок за день, 12 SL.
- WIFUSDT 4/4 в SL, **WR=0%**, NET −$6.74.
- SOLUSDT 3/3 в SL, **WR=0%**, NET −$2.80.
- TONUSDT 5 сделок 01.05, 1W/4L, NET −$6.27 (за всю Wave 6 TON: −$2.05).
- Концентрация в окне 19-21 UTC NY (vwap_short активные часы).
- Серия из 3-4 одинаковых сигналов на одном символе подряд за 1 час
  («открыл → SL → 5 мин → новый сигнал → SL»).

**Это ВТОРОЙ независимый «alt-selloff regime event»** за 10 дней.
Первый — 22.04 (запись `BYBIT_AB_TEST.md` OBSERVATION 2026-04-23):
TIA −7.8%, SOL −2.8%, BTC стационарный, scalp_vwap 4 Long на TIA
подряд за час, все 4 SL.

Паттерн идентичный (по обоим эпизодам):
1. BTC стационарный или слабый.
2. Альты валятся независимо от BTC.
3. mean-reversion long открывается «на провале».
4. Провал продолжается → SL.
5. Следующий цикл — снова сигнал на тот же символ → снова SL.

**Биномиальный тест (sample-size.mdc):**
- WR 36.8% vs H0=55% (Wave 6 ожидание) при n=19: z=−1.60, p≈0.055
  (двусторонний 0.11). Не значимо при α=0.05.
- WR 36.8% vs H0=50% при n=19: p≈0.25. Не значимо.

По правилам **ничего не отключаем и не правим**: n=19 << 100, период
< 2 недель. Тенденция настораживает, но статистики недостаточно.

**Технический шум в БД (для recon-логики, не торговая):**
- БД: 23 записи vs API: 19. Расхождение:
  - 3 × `sync_orphan` (closedPnl не дождался за 30мин, БД помечает 0).
  - 1 × дубль (TON 01.05 20:30 + 20:35 — идентичные entry/exit/pnl,
    БД создала 2 записи, API одну).
- Корректировки `_reconcile_pending_pnl` работают, но дубль —
  возможный баг создания позиции.

**Рабочие гипотезы для T+14d среза (09.05) — НЕ внедряем сейчас.**

Расшифровка терминов простым языком (для будущей навигации по логу):

- **T+14d** — «через 14 дней после старта Wave 6». Wave 6 деплой
  25.04 → срез 09.05. До этой даты по `sample-size.mdc` нельзя
  менять параметры (мало данных).
- **HTF-trend filter** — фильтр старшего таймфрейма (часовик). Если
  по 1h явный нисходящий тренд — не лонгуем. Сейчас фильтр **есть,
  но мягкий**: блокирует только при сильном наклоне. Жёсткий
  вариант — блокировать любой вход против H1-направления.
- **Dynamic ADX_MAX** — динамический порог ADX (индикатор силы
  тренда). Сейчас фиксированный 25 для VWAP. Динамический =
  снижать порог в дни/часы высокой волатильности (входить только
  когда совсем флэт).
- **BTC-correlation monitor** — следим за BTC. Если BTC падает и
  наш альт скоррелирован с BTC — не лонгуем альт. Сейчас фильтра
  нет вообще.
- **Per-symbol cooldown после SL** — после стоп-лосса запретить
  новые входы по тому же символу N минут (≈60). Защищает от серии
  «4 long на WIF за час, все SL» (как 01.05 и 22.04).

При повторе паттерна на T+14d (3-й эпизод) — обсуждать research
со ссылками на источники:

1. **HTF-trend filter (жёсткий вместо мягкого).** Сейчас 1h
   EMA50-slope блокирует только при `|slope|>0.0005`. Идея —
   снизить порог или сделать абсолютно блокирующим вход против
   среднесрочного тренда.

2. **Dynamic ADX_MAX.** Сейчас VWAP вход разрешён при ADX(14)≤25
   (статика). Идея — снижать порог в дни/часы повышенной
   волатильности (BTC daily range >1%, или 1h RV спайк), оставаясь
   высоким во флете.

3. **BTC-correlation monitor.** Если BTC падает (1h move ≤−0.5%) и
   корреляция альт vs BTC ≥0.7 на 24h log-returns — блокируем LONG
   альта. Сейчас этого фильтра нет.

4. **Per-symbol cooldown после SL.** Самая простая защита от серии
   одинаковых сигналов: после SL запретить новые входы по тому же
   символу N минут (например 60). Защищает от паттерна «4 long на WIF
   за час, все SL».

Все четыре требуют:
- backtest на 90-day данных (январь-апрель 2026),
- in-sample / OOS разбивка с минимум 10 weeks в OOS,
- прогон через `scripts/backtest_all.py` или новый специализированный,
- обсуждение с пользователем по `strategy-guard.mdc`.

**Метрики для T+14d (09.05) среза:**
- Повторился ли alt-selloff паттерн (3-й эпизод)? — основной критерий.
- WR scalp_vwap на n≥30 — стабилизировалась к ~50% или системно
  ниже?
- Доля «whipsaw-серий» (3+ losses подряд на одном символе за 1ч).
- Concentration risk: какой % убытков приходится на 1 худший день?
  (сейчас 100% убытков от 01.05 при +$1.02 на остальных 4 днях).
