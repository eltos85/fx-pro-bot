...
#
# Значение выведено из данных, а не выбрано: медиана NATR по 93 символо-дням
# объединённой популяции с cutoff 2026-07-22T14:08:00Z, посчитана 2026-08-06.
# Порог привязан к поколению эксперимента: меняется cutoff → пересчитать руками
# и в коммите (отчёт печатает актуальную медиану рядом с замороженной, но сам
# константу не трогает — иначе воспроизводимость снова теряется).
NATR_SPLIT = 0.385209
