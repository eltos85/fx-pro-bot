...
### Множитель ширины SL (decoupled, v0.18.8)

- `sweep_fade_sl_risk_mult` (в проде env = **2.0**). Расширяет ТОЛЬКО SL
  (`sl_risk = base_risk × 2.0`); TP и flow_exit остаются на `base_risk` (см.
  «Выходы» и общую формулу TP). density_break НЕ задет (`sl_mult=1.0` прибит).
  - [research — ширина] MAE/Sweeney «Maximum Adverse Excursion» (1988): стоп за
    85–90-м перцентилем MAE винеров. MR + стоп: тугой (1 ATR) убивает edge, шире
    сохраняет (aligrithm 3.27 2026); TradeMedic 500k счетов — выбивания шумом
    главный слив (hoc-trade 2026).
  - [research — decoupling] SL = MAE (шум), TP/flow_exit = MFE (ход в плюс) — две
    НЕЗАВИСИМЫЕ статистики; MFE от ширины стопа не зависит, поэтому привязки
    разведены. Иначе цель уезжала бы вместе со стопом (была подгонка).
  - [наш A/B — выбор ×2.0 двумя сошедшимися методами, НЕ «где netR выше»]:
    1) MAE-винеров (n=412, много режимов, `data/scalp_sf_mae_out.txt`): p85=1.87R,
       p90=2.39R → канон-ширина ≈2.0 (×1.5 сохраняет лишь 78% винеров, ниже полосы;
       ×2.0 = p85, 85%; ×2.5=p92 щедро).
    2) decoupled harness июнь 1–4 (`data/scalp_sl_decouple_ab.txt`): netR ×1.0 −73R
       → ×1.5 −7R → ×2.0 +15R. Сходимость с MAE-перцентилем снимает overfit.
  - [наш A/B] 117 реальных стоп-вылетов (30.05–04.06): ×2.0 ~35% выбиваний были
    шумом и развернулись бы в плюс.
  - [forward-test] эдж на live ещё накапливается (≥100 сделок / ≥2 недели). Откат:
    env `SCALP_SWEEP_FADE_SL_RISK_MULT=1.0`.
