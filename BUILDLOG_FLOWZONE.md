...
(`4bae481^`). В compose снова отдельный сервис `flowzone-bot` со своим томом
`flowzone_data`. Hybrid не трогаем. Стратегию не меняли.

**Файлы:** `src/flowzone_bot/`, `Dockerfile.flowzone-bot`,
`tests/test_flowzone_bot.py`, `docker-compose.yml`, `pyproject.toml`,
`STRATEGY_FLOWZONE.md`, `TASKSPEC_FLOWZONE.md`.
