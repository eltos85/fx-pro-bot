...
запуске. После v0.18.55 в популяцию вошли 22 counterfactual-дня (они заметно
волатильнее живой вселенной), медиана уехала 0.367→0.432, и четыре символо-дня
из 73 сменили ярлык: HYPE 04.08 (NATR 0.422), ETH 28.07 (0.394), SOL 03.08
(0.380), ETH 29.07 (0.372) — все в узкой полосе между старым и новым порогом.
Именно они держали ячейку.
...
Решение: `NATR_SPLIT = 0.432055` заморожен в коде. Значение выведено из данных
(медиана 73 символо-дней объединённой популяции с cutoff 2026-07-22), а не
...
|---|---|---|---|
| shadow_signals (живая вселенная) | 59 | 0.362 | 66% |
| counterfactual (тени) | 22 | 0.574 | 18% |
...
как предсказывалось (в общей карте 23 «тренд/тихо» и 13 «флет/тихо» из 73).
`density_break v2` 2/4→3/4 режимов, `density_break_meta_gate` 2/4→3/4,
теневые `sweep_fade` и `density_bounce` 1/4→2/4. `sl_widen_x3` дособрал
кластеры (39→43) и стал READY — эксперимент с шириной стопа закрыт полностью,
по всем четырём веткам; выводов это не меняет (см. `d84cac2`).
