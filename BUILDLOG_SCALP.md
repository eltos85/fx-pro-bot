...
### TODO (наблюдение, без правки кода) — асимметрия отбора монет под long/short
Гипотеза: универсум-селектор (отбор символов в ротацию) может по-разному
подходить лонгам и шортам — возможно монеты, дающие хорошие шорт-фейды, и монеты
для лонг-фейдов различаются (на даунтренд-альтах шорты системно лучше, v0.18.4).
Проверить: разбить отобранные символы по тому, на какой стороне они дают edge
(live + харнес by-symbol×by-side), и подумать о направленно-зависимом отборе.
Пока НЕ трогаем (sample-size: нужна выборка ≥100 на связку символ×сторона).
