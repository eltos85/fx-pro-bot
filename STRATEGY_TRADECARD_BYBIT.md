...
(Bybit, orderflow sweep/density) и **`hybrid_bot`** (Bybit, регулярная фиксация
тренда от средней цены входа). Универсальный канон фреймворка (для LLM-агентов) живёт в
...
> `STRATEGY_*` / `STRATEGY_HYBRID.md`). Это **процесс развития** торговой
...
`hybrid_bot` исполнитель — **детерминированный движок без дискреции**. Поэтому
...
- `hybrid_bot`: `score` не используется (у стратегии нет набора факторов входа,
  STRATEGY_HYBRID.md §17.4) — грейдинг по score к нему не применяется.
...
- `hybrid_bot` — пока **один** playbook (`hybrid_fix_from_avg`).

Маппинг канона:

- **Один playbook → ≈4 → 18-25.** Развивающейся системе нужно ≈4 рабочих
  playbook'а под разные режимы; `hybrid_fix_from_avg` (один) — по канону «нужно быть в нём
...
- `TASKSPEC_TRADECARD_BYBIT.md` — ревьюер над данными `scalp_bot` и
  `hybrid_bot` (Bybit, детерминированные).
