...
улучшению. Победа засчитывается ТОЛЬКО когда:
  (а) гипотеза была одобрена человеком и **внедрена** (у неё проставлена
...
from tradecard_bybit.analysis.stats import two_proportion_test
...
def evaluate_small_win(db: TradecardDB, *, hypothesis_id: int, theme_id: int,
    ...
    # победа: значимое СНИЖЕНИЕ частоты (diff<0) на OOS
