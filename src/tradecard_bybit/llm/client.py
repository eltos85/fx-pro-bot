"""DeepSeek-клиент tradecard_bybit (Anthropic-compatible API), read-only.

Инлайн-копия клиента fx_ai_trader (`src/fx_ai_trader/llm/client.py`) — у
Bybit-ботов своего LLM нет, клиент живёт ВНУТРИ пакета без кросс-зависимости
(TASKSPEC §2/§6/§9). Используется **только** для аналитики 5 Why, ничего не
меняет в торговле.
...
log = logging.getLogger("tradecard_bybit.llm")

# Грубые цены для приблизительного учёта стоимости (не биллинг).
...
class DeepSeekClient:
    ...
    def ask(self, system_prompt: str, user_prompt: str) -> LlmResponse:
        ...
        # Fallback без thinking — thinking-блоки могли съесть весь бюджет.
