"""Событийные датчики для AI-Trader (Bybit perpetual-futures).

Порт инструмент-независимой логики из ``src/fx_ai_trader/trading/
price_sensor.py`` (2026-05-29). Импорт из fx-пакета запрещён правилом
изоляции (strategy-guard.mdc «fx_pro_bot.* и bybit_bot.* — две
независимые экосистемы»), поэтому здесь — самостоятельная копия.
Логика чисто математическая (R-units + state-machine на rising-edge),
без привязки к классу актива.

Поверх живого spot-стрима (``price_stream.py``) датчики дают
event-driven реакцию — будят внеплановый вызов аналитика по факту
рыночного события, не дожидаясь планового таймера:
- ``LockedProfitSensor`` — позиция дошла до зоны locked-profit
  (≥ threshold_r) → внеплановый REVIEW (guardian защищает прибыль).
- ``AdverseMoveSensor`` — позиция ушла в минус на ≥ threshold_r →
  внеплановый FULL (стратег с macro пересматривает тезис).
- ``EntryBreakoutSensor`` — живая цена пробила Donchian-канал →
  внеплановый FULL (аналитик с macro/news решает open/hold).

Защита от шума и расходов:
- rising-edge + гистерезис: срабатывает на ВХОДЕ в зону, затем
  «обезоруживается» пока R не вернётся за порог − гистерезис.
- cooldown_sec: минимум времени между событиями.
- max_events_per_hour: жёсткий потолок внеплановых вызовов за час.

Research basis:
- Sutton & Barto «Reinforcement Learning» (2018) §3 — event-driven
  реакция эффективнее фиксированного опроса при разреженных событиях.
...
- Donchian (1960s); Faith «Way of the Turtle» (2003) — 20-period
  channel breakout, каноничный lookback (EntryBreakoutSensor).
"""
...
class AdverseMoveSensor:
    ...
    Когда открытая позиция уходит в минус на ≥ threshold_r, будит
    внеплановый FULL-цикл — чтобы стратег с macro пересмотрел тезис
    (broken/intact). Согласуется с review-guardian: тезис судит full
    с macro, а НЕ review-guardian по голому 1H. Раньше full-цикл ждал
    до 15 мин; теперь реагирует по факту движения.
...
class EntryBreakoutSensor:
    """Датчик возможности входа: пробой Donchian-канала живой ценой.

    Дешёвый «наблюдатель за экраном». Когда живая цена пробивает
    20-баровый (1H) максимум/минимум (Donchian channel), будит
    внеплановый FULL-цикл — аналитик с macro/news решает open/hold.
