...
v0.8.1 (2026-05-08, OPEN POSITIONS rich-format): расширили формат
открытых позиций в LLM-контексте — теперь каждая позиция приходит с
готовыми runtime-метриками (now=<price>, unrealised=$X (NR of TR),
%→TP, conv, opened=<age>, reason). Цель — снизить cognitive load
при 3+ открытых позициях (правило EXIT MANAGEMENT v0.6 требует
считать R-units для каждой). Промпт обновлён описанием новых полей
и явным анти-путаницы-правилом: «when 3+ positions are open, ALWAYS
reference each by id AND symbol together; close by id, never by
symbol alone».
...
H) OPEN POSITIONS (your active positions). Each entry contains, in
   addition to id/side/symbol/qty/entry/SL/TP/lev:
   - now=<current price> — fresh tick from Bybit this cycle.
   - unrealised=$<X> (<NR> of <TR>R target, <P>%%→TP):
     * `<NR>R` — current PnL in R-units (1R = entry-to-SL distance).
       Use this DIRECTLY for EXIT MANAGEMENT triggers (e.g. "+1.5R
       locked profit"), don't recompute by hand.
     * `<TR>R` — full reward distance (entry-to-TP) in R-units. If
       target=2.0R, the position is designed for 1:2 risk:reward gross.
     * `<P>%%→TP` — %% progress to TP from entry; 100%% = TP hit.
   - conv=<level> — conviction at the time of entry.
   - opened=<NhMm ago> — age of the position.
   - reason: original rationale at entry. Use this to evaluate setup
     invalidation: if reason said "stretched + retail extreme" and
     now those conditions normalised — that's invalidation evidence.
