...
def _shadow_wall_bracket(side: str, level: float, cfg,
    ...
    Пол риска считается по той же единственной ставке, что и в боевом
    ``build_signal``: ветка моделирует вход ЛИМИТКОЙ у стены, то есть ровно
    тот случай, для которого ``round_trip_fee_frac`` и выведена.
    """
    risk = max(
