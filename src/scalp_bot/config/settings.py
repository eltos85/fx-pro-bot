...
class ScalpSettings(BaseSettings):
    ...
    # sweep_fade_run (v0.18.27, 2026-06-26) УДАЛЕНА v0.18.37 (2026-07-15,
    # решение пользователя): форвард n=176 WR 12% net −$327 — гипотеза «дай
    # winners бежать» опровергнута. Возврат к канону: sweep_fade_canon (A/B
    # base vs canon, как в исходном дизайне v0.18.20). Числа — BUILDLOG_SCALP
    # 2026-07-15.
    # sweep_fade_canon отключалась v0.18.59 и ВОЗВРАЩЕНА v0.18.65 (2026-08-10,
    # решение пользователя). Отключение опиралось в том числе на довод «валовой
    # −0.112R, снижение комиссии не спасает»; довод снят как неверный (1a90ad2):
    # он считался по телеметрии fees_usd, покрытие которой перекошено между
    # стратегиями (30% у канона против 8% у sweep_fade, т.к. maker-филлы часто
    # приходят без ставки). По единой модели ставки валовой канона на 60 днях
    # +0.086R — ВЫШЕ, чем у density_break (+0.037) и sweep_fade (+0.022); все
    # интервалы включают ноль, то есть по валовому канон не хуже остальных.
    # Разрыв в чистом (−0.275 против −0.194) ≈ разница ставок taker-входа
    # (0.360 против 0.216R): канон проигрывал ценой входа, а не идеей.
    # Возвращена ради разнообразия символов: она единственная торговала
    # мейджоров, без неё вселенная схлопнулась до двух монет (BUILDLOG v0.18.63)
    # и мы перестали набирать данные по глубоким книгам. Счёт демо, минус
    # принят осознанно как цена наблюдений.
    # density_break ОТКЛЮЧЕНА v0.18.67 (2026-08-24, решение пользователя) —
    # единственная страта, взявшая все условия sample-size.mdc: n=327,
    # 118 символо-дней, 85 дней, cleanR −0.323 CI [−0.522; −0.124] p=0.0015,
    # размер эффекта |R|≥0.3. Артефакт: scripts/scalp_sample_readiness_check.py.
    # Анатомия: выходов ровно два — 76.8% стоп, 23.2% тейк, дискреционных нет.
    # Медианный стоп −1.37R и тейк +3.17R вместо −1.0/+3.5 — разница есть
    # комиссия. Безубыток требует WR 30.2% при фактическом 22.9%; при нулевой
    # комиссии требовал бы 22.2%, то есть страта окупалась ТОЛЬКО бесплатно.
    # Валовой край +0.032R CI [−0.166; +0.230] — не отличим от нуля, защищать
    # нечего. Мейкер-вход проверен и снят: экономит 2.5 п.п. из нужных 8.6,
    # исполняемость 43.8% в собственный мейкерский период (до v0.18.16), и WR
    # тогда был 22.2% — не выше нынешнего, что указывает на отбор (лимитка на
    # пробое наливается на ложных). BUILDLOG_SCALP 2026-08-24.
    enabled_strategies: str = Field(
        default="sweep_fade,density_bounce,sweep_fade_canon")
    ...
    # ─── rolling-regime lookback (бывш. sweep_fade_trend, v0.18.27) ─────────
    # sweep_fade_trend удалена v0.18.33, но rolling regime_ratio ОСТАЁТСЯ:
    # KeyLevels считает его для regime_features-телеметрии (meta-labeling) и
    # shadow-лога. Lookback rolling-regime (закрытых 15m-баров). 8 = 2 часа —
    # консистентно с day_regime в scripts/scalp_canon_study.py.
    regime_ratio_lookback_bars: int = Field(default=8)
    ...
    # ─── Теневая вселенная (v0.18.48) ────────────────────────────────────
    # Наблюдение за монетами, которые отсекает ТОЛЬКО порог оборота
    # (universe_min_turnover_usd), хотя range- и spread-стражи они проходят.
    # Повод (измерение 2026-07-29, /tmp/scalp_universe_measure.py):
    #   • авто-вселенная выродилась хронически: 07-08..07-23 торговался ровно
    #     один ZECUSDT, 11 из 16 дней набор не менялся, с 07-28 17:22 МСК
    #     фильтр отдаёт 0 монет (173 предупреждения в логе);
    #   • на текущем снимке 299 монет отсечены только оборотом, но 94 из них
    #     проходят spread-страж — их стоимость исполнения НЕ хуже боевой;
    #   • гипотеза «на низколиквидных теряем» НИ РАЗУ не проверялась: 439 из
    #     470 сделок (93%) прошли при спреде <1 bps, и именно эта корзина даёт
    #     netR −0.127. Классическое range restriction.
    # Порог оборота НЕ меняем: сначала измеряем исходы тенью (evidence-first,
    # как canon_rejection_shadow и sl_widen). Тени НЕ создают ордеров.
    shadow_universe_enabled: bool = Field(default=True)
    # Кэп символов: WS-подписки для linear не лимитированы по числу args, но
    # массив args ограничен 21 000 символов, а CPU растёт линейно по стакану
    # (https://bybit-exchange.github.io/docs/v5/ws/connect — «No args limit for
    # Futures», «cannot have length of args array over 21,000 characters»).
    # Замер 2026-07-29: 9 символов = 19.5% CPU при 2 ядрах, load 0.67 → 94
    # символа насытили бы обе ядра. 6 — запас по CPU (~+65% к текущему) и
    # достаточный темп: до порога 100 исходов ≈ 2 недели, что совпадает с
    # min_days=14 в forward-checkpoint.
    shadow_universe_max_symbols: int = Field(default=6)
    # Пол оборота для самих теней: наблюдать «почти дотянувших», а не пыль.
    # $10M — на порядок ниже боевого $100M, но выше типичной микрокапы.
    shadow_universe_min_turnover_usd: float = Field(default=10_000_000.0)
    ...
    # v0.18.58: ветки ширины стопа для density_bounce. Замер 2026-08-08 (после
    # вычета фантомных заливов, 5357 теневых исходов): это ЕДИНСТВЕННАЯ
    # стратегия с валовым краем выше безубытка — +0.111…+0.207R при безубытке
    # 40% и нижней границе CI выше него на всех четырёх рубежах. Не хватает
    # только комиссии: стоп 0.30% от цены превращает round-trip 0.075%
    # (maker-вход + taker-выход) в 0.250R. Комиссия в R = ставки / ширина
    # стопа, поэтому растяжение стопа при том же R:R — прямой рычаг.
    # sl_widen на боевых сделках (v0.18.54) дал отрицательный результат, но
    # density_bounce в ту выборку не попала: она молчит, а у density_break
    # знак эффекта был противоположный (v0.18.55 finding). Меряем отдельно.
    # 0 отключает ветки; 90 — рубеж с лучшим сочетанием выборки и края.
    density_bounce_sl_shadow_at_persist_sec: float = Field(default=90.0)
    ...
    #
    # Ставка верна ТОЛЬКО для maker-входа, и это сознательно оставлено так.
    # Замер 31.08 (`scripts/collect_scalp_stats.py --days 30`, 969 филлов,
    # комиссия из openFee+closeFee со сверкой по execFee): лимиткой входит
    # одна стратегия из четырёх — мейкерских филлов 93 из 641, у sweep_fade
    # 49%, у sweep_fade_canon / density_break / density_bounce ровно 0%.
    # Фактическая ставка в долях риска сходится с обеими моделями: sweep_fade
    # 0.227R × 0.300% = 0.068%, рыночные 0.349–0.368R × 0.300% = 0.105–0.110%.
    # Средняя по счёту — 0.1006% за круг против 0.075% здесь.
    #
    # Подставлять фактическую ставку в гейты НЕ нужно, и это измерено дважды:
    # `71a11ed` (30.07) отложил вопрос, `2026-08-03` закрыл его — расширение
    # стопа рыночным стратегиям с 0.30% до 0.44% цены даёт ΔnetR −0.007
    # [−0.211; +0.197] и +0.081 [−0.139; +0.301] (ветки x1.5 и x2 sl_widen).
    # Комиссия в R падает втрое, но валовой результат ухудшается настолько же.
    # Инвариант «fee ≤ 0.25R» ниже поэтому не выполняется у market-входов
    # (факт 0.335R) — это известное и принятое расхождение, а не недосмотр.
    ...
    #
    # Замер 31.08 (`scripts/scalp_fee_constant_impact.py`): этот гейт НЕ
    # отбрасывает ничего и не может — 0 срабатываний на 310 живых сделках и
    # 46 862 теневых сигналах. Цель ставится как `take_profit_r × R`, а R
    # снизу ограничен полом `min_risk_fee_mult × round_trip`, поэтому условие
    # «цель < 3 × round_trip» требует `take_profit_r < 3/4`; в бою она 2.5–3.5.
    # Минимальный ход до цели в выборке — 1.000% при планке 0.225%. Гейт
    # избыточен при любой ставке (правка ставки масштабирует обе стороны
    # сравнения), но оставлен: он страхует от конфигураций с малым
    # take_profit_r и стоит один if. Реально геометрию связывает пол ниже.
    ...
    # fee ≤ 1/mult доля R. mult=4 → fee ≤ 0.25R (R ≥ 0.300% цены, TP 3.5R ≈
    # 1.05% — центр проф-коридора цели скальпа 0.5–2%). Именно этот пол, а не
    # fee-guard выше, связывает геометрию на практике: замер 31.08 показал, что
    # стоп стоит ровно на нём у 233 из 310 живых сделок (75%) и у 87% теневых
    # сигналов, то есть у большинства сделок дистанцию задаёт не свипнутый
    # уровень, а эта константа. Обоснование (research, не подгонка под
    ...
    # ОЧИЩЕНЫ v0.18.67 вместе с отключением density_break: пины force-include
    # символы в WS, а гейт в main отдаёт их ТОЛЬКО density_break. Оставить их
    # при выключенной страте значило бы держать подписки на монеты, которые не
    # торгует никто, и заодно прятать их от sweep_fade/density_bounce.
    density_break_pin_symbols: str = Field(default="")
