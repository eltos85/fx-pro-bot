...
class ScalpSettings(BaseSettings):
    ...
    # 2026-07-15.
    # sweep_fade_canon ОТКЛЮЧЕНА v0.18.59 (2026-08-08, решение пользователя).
    # Единственная стратегия, прошедшая все четыре порога sample-size.mdc для
    # отключения: 114 сделок (порог 100), 17 дней (порог 2 недели), чистый R
    # −0.413 при CI95 [−0.665; −0.182] — ноль исключён, величина кратно больше
    # порога 0.3R. Убыточна во ВСЕХ четырёх сессиях входа (−0.33 / −0.40 /
    # −0.52 / −0.31R), то есть дело не во времени суток (сессионная гипотеза
    # отклонена out-of-sample, BUILDLOG cafdd33). Валовой результат −0.112R,
    # то есть снижение комиссии её не спасает: нечего спасать. Итог −$468.91.
    enabled_strategies: str = Field(
        default="sweep_fade,density_bounce,density_break")
