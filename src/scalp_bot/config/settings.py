...
class ScalpSettings(BaseSettings):
    ...
    # ─── Управление позицией ─────────────────────────────────────────────
    # Тайм-стоп: скальп не должен «висеть», но победителю нужно ВРЕМЯ добежать.
    # 60→120с (Философия B 2026-05-31, BUILDLOG_SCALP v0.7.0). Раньше 60с резал
    # ради ограничения time_stop-убытка (86% потерь по анализу 304 сделок), но
    # теперь ранний срез УБЫТОЧНЫХ делает flow_scratch (≥20с при развороте ленты),
    # поэтому тугой 60с-бэкстоп больше не нужен как защита от убытка — он лишь
    # душил РЕДКИЕ крупные вины (медиана выхода победителя 55–67с, до 3.5R нужно
    # больше). 120с в каноне хвата order-flow скальпа (Kalena DOM 2026: 10–90с+).
    time_stop_sec: float = Field(default=120.0)
