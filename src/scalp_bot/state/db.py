...
class ScalpDB:
    ...
    # Закрытия без биржевого closedPnl (нечего сверять). Префикс `entry_`
    # неслучаен: аналитические скрипты фильтруют по `close_reason NOT LIKE
    # 'entry_%'`, поэтому новая техническая причина исключается из статистики
    # везде сразу, без правки каждого запроса.
    #
    # `entry_netted` — исторический ярлык: до `c9effac` (24.08) наш вход против
    # чужой стороны на общем one-way счёте не открывал позицию, а срезал чужую
    # (офдок position-mode: один лот на символ). Позиции не было, значит не было
    # и результата — но строка в БД жила и «сопровождалась». #4325 так набрал
    # выдуманные −$289 при том, что на бирже в ту секунду закрылся ЧУЖОЙ лонг с
    # +$21.96. Сейчас такая строка не появляется: вход отменяется до постановки
    # ордера, поэтому ярлык нужен только для чистки истории.
