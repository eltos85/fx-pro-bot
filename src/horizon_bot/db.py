"""SQLite: наши позиции и сделки. Чужие позиции на том же счёте не трогаем."""
...
class HorizonDB:
    def __init__(self, path: str) -> None:
        ...
        self._db.execute(
            """CREATE TABLE IF NOT EXISTS trades (
                 ...
                 entry REAL, exit REAL, pnl_usd REAL,
                 reason TEXT
               )""")
        self._db.commit()
    ...
    def owned(self, symbol: str) -> dict | None:
        row = self._db.execute(
            "SELECT symbol, side, qty, entry, ts_open, link_id "
            "FROM positions WHERE symbol=?", (symbol,)).fetchone()
        if not row:
            return None
        keys = ("symbol", "side", "qty", "entry", "ts_open", "link_id")
    ...
    def open_pos(self, symbol: str, side: str, qty: float, entry: float,
        ...
        self._db.execute(
            ...
            "(symbol, side, qty, entry, ts_open, link_id) VALUES (?,?,?,?,?,?)",
            (symbol, side, qty, entry, int(time.time()), link_id))
        self._db.commit()
