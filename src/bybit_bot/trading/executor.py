...
class TradeExecutor:
    ...
    def execute(self, params: TradeParams) -> OrderResult:
        """Отправить ордер: Limit PostOnly (maker 0.02%) с fallback на Market."""
        sl = params.sl
        tp = params.tp
        ...
        if last_price > 0 and not params.force_market:
            ...
            log.info("Limit PostOnly %s %s qty=%s price=%s",
                     params.side, params.symbol, params.qty, limit_price_str)
            result = self._client.place_limit_order(
                params.symbol, params.side, params.qty, limit_price_str,
                sl=sl, tp=tp,
            )
            if result.success:
                return result
        ...
        log.info("Market %s %s qty=%s SL=%.4f TP=%.4f",
