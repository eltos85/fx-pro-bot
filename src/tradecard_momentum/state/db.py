"""Собственная SQLite tradecard_momentum: темы, гипотезы, частоты, победы.

Это **единственное** хранилище, в которое tradecard пишет. В БД momentum-бота он
НЕ пишет ничего (read-only инвариант). Изолированная копия схемы tradecard_bybit
(пакет самостоятельный, без кросс-импортов — TASKSPEC §9). Хранит:
...
Продвижение гипотезы в реальное изменение конфига momentum-бота = только человек,
