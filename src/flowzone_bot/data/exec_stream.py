"""Bybit private WebSocket → realtime executions (flowzone_bot).
...
- ``execPnl``   — realized P&L закрывающего филла;
- ``execFee``   — фактическая комиссия именно этого филла;
- ``execPrice``/``execQty`` — реальная цена/объём филла;
- ``orderLinkId`` — наш тег ордера → точный матч к сделке без гонок по времени;
- ``closedSize`` — закрытый объём (>0 у закрывающего филла).

net сделки = Σ execPnl − Σ execFee по всем филлам позиции (= Bybit closedPnl).

Приватный demo-домен включается флагом ``demo``. Колбэк pybit исполняется в
отдельном треде → складываем в потокобезопасную очередь, главный цикл забирает
``drain()`` и атрибутирует к сделкам (в своём треде).
"""
...
log = logging.getLogger("flowzone_bot.exec_ws")

# Только реальные торговые филлы несут позиционный P&L. Funding/Settlement —
# отдельные кэшфлоу, в per-trade closedPnl Bybit их НЕ включает.
