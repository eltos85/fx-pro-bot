...
``classify`` — МГНОВЕННЫЙ режим по накопленному дневному профилю (§6.3 «Dly Vol.
Profile»). Решение: из объёма, принятого ВНЕ value area (хвосты выше VAH и ниже
VAL), доля ≥ ``accept_frac`` на одной стороне → это направление аукциона.
``accept_frac`` = 0.70 — каноничная Value-Area-константа (Steidlmayer/Dalton:
value area ≈70% принятого объёма; «acceptance» вне VA — направленное принятие той
же грейд-доли), reversible через env (no-data-fitting.mdc).
...
только по встречному структурному пробою — это вынесено в ``auction.AuctionTracker``
(латч + «второе движение»). ``classify`` остаётся чистой функцией (формы профиля),
а латч-логика — поверх неё, чтобы обе части тестировались раздельно.
"""
...
def classify(profile: VolumeProfile | None, last_price: float | None, *,
    ...
    ``auction.AuctionTracker`` поверх этой функции. ``accept_frac`` = 0.70 —
    каноничная Value-Area-константа.
    """
