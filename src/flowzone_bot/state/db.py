...
Потокобезопасность: соединение открыто с ``check_same_thread=False`` и делится
между main-циклом (сделки, профили сессий) и фоновым потоком флаша принтов.
Офдок sqlite3: *«If False, the connection may be accessed in multiple threads;
write operations may need to be serialized by the user to avoid data
corruption»* (https://docs.python.org/3/library/sqlite3.html#module-functions).
Сериализуем сами: каждый метод целиком под ``self._lock``. Лочить отдельные
``execute``/``commit`` НЕ достаточно — sqlite3 в legacy-режиме открывает
транзакцию неявно на первом DML, поэтому пара (DML → commit) должна быть
атомарной; иначе потоки чередуются и получаем «cannot start a transaction
...
class FlowzoneDB:
    def __init__(self, data_dir: str) -> None:
        ...
        # RLock (не Lock): _migrate вызывается из __init__ уже под локом.
    ...
    def _migrate(self) -> None:
        ...
        with self._lock:
            ...
            if "zone_low" not in cols:
                self._conn.execute(
                    "ALTER TABLE trades ADD COLUMN zone_low REAL")
            if "zone_high" not in cols:
                self._conn.execute(
                    "ALTER TABLE trades ADD COLUMN zone_high REAL")
    ...
    def mark_closed(
        ...
        with self._lock:
            self._conn.execute(
                "UPDATE trades SET status='closed', ts_close=?, exit=?, "
                "pnl_usd=?, fees_usd=?, close_reason=?, pnl_provisional=? "
                "WHERE id=?",
    ...
    def prints_count(self) -> int:
        with self._lock:
            row = self._conn.execute(
                "SELECT COUNT(*) AS c FROM prints").fetchone()
    ...
    def prune_session_profiles_before(self, before_start: float) -> int:
        ...
        with self._lock:
            cur = self._conn.execute(
                "DELETE FROM session_profiles WHERE session_start<?",
                (before_start,))
            self._conn.commit()
            return int(cur.rowcount or 0)
    ...
    def provisional_closed_since(self, ts: float) -> list[TradeRow]:
        with self._lock:
            rows = self._conn.execute(
                ...
                "AND pnl_provisional=1 AND ts_close>=? ORDER BY id", (ts,)
            ).fetchall()
