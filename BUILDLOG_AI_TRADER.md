...
## 2026-05-11 — hotfix(v0.13.1): rr_net_fee 1.8 → 1.5 (откат самовольного ужесточения)

`<hash-pending>`

**Симптом.** В логах LLM прямо процитировал «rr_net_fee < 1.8 on potential
entries» как причину HOLD на ATOMUSDT. Пользователь спросил: «а почему R:R
>= 1.8? было вроде R:R >= 1.5?».

**Корневая причина — моя ошибка в v0.11.1.** Когда я добавлял compliance
sub-object в JSON-схему, я **самовольно** написал `rr_net_fee >= 1.8`
(и в FAILS-условии — `< 1.8`). Это противоречило базовому правилу
промпта `R:R >= 1.5` (строки 131-132, 144-145, 288, 312, 456 prompts.py),
который существовал давно и опирается на канон **Van K. Tharp «Trade
Your Way to Financial Freedom» (2007, ch.11)** и **Ralph Vince «The
Leverage Space Trading Model» (2009)** — индустриальный стандарт для
крипто-перпов.

Это **нарушение `strategy-guard.mdc`**:
- ЗАПРЕЩЕНО менять research-based параметры без ссылки на новый источник.
- ЗАПРЕЩЕНО «подкручивать» пороги интуитивно или ради прохождения тестов.

Я ввёл порог 1.8 без research-обоснования. LLM применяла более строгое
правило из compliance (так как это структурированное обязательное поле,
а не «soft suggestion»), и блокировала любые сетапы с R:R 1.5..1.8 —
это типичный диапазон mean-reversion сделок. Из-за этого, частично,
концентрация активности на узком пуле пар: крупные капы редко дают R:R
>= 1.8 на коротких mean-reversion сетапах, а альты с высоким ATR — да.

**Изменения.**

- `src/ai_trader/llm/prompts.py`: в JSON-схеме `compliance.rr_net_fee`
  возвращено `REQUIRED >= 1.5` (синхрон с текстовым правилом). В FAILS-
  условии — `rr_net_fee < 1.5`. Никаких других изменений промпта.
  В docstring модуля добавлена запись v0.13.1 с явным признанием
  ошибки и ссылкой на нарушенное правило `strategy-guard.mdc`.

**Тесты.** +1 regression-guard `test_prompt_rr_net_fee_threshold_is_1_5`
который ассертит `rr_net_fee < 1.5` в FAILS-блоке и **не пропускает**
`rr_net_fee < 1.8`. С пояснением в docstring почему этого изменения
быть не должно (для будущего меня и code-reviewer).

Итог: `pytest tests/` — 608 passed (было 607 → +1). Линтер чист.

**Эффект на торговлю:**

- Сетапы с R:R 1.5..1.8 (mean-reversion на крупных капах: BTC/ETH/BNB
  с тугим TP/SL) снова проходят compliance.
- Откат к поведению до v0.11.1 в части R:R-фильтра.
- Не должно увеличить количество plохих сделок — мы возвращаемся к
  каноническому 1.5 (Tharp/Vince), а не «ослабляем» что-то.

**Что наблюдать.** Распределение пар в OPEN-решениях LLM (см. анализ
`/tmp/symbol_distribution.py`): до v0.13.1 BTC/ETH/BNB/DOGE редко
открывались (n=2..3, BNB=0). Если за неделю эта картина изменится в
сторону более равного распределения — фикс работает.

**Lessons learned для меня.** Любое числовое значение, которое я
добавляю в стратегию (порог, множитель, диапазон), должно либо
**копировать** существующий канонический параметр, либо **ссылаться
на research** в комментарии рядом с числом. Если такой ссылки нет —
это data-fitting / intuition, что прямо запрещено `strategy-guard.mdc`
и `no-data-fitting.mdc`. В этом коммите нарушения не должно произойти
ровно потому что я возвращаю значение к канону, а не двигаю его.
