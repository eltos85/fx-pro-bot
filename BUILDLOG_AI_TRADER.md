...
**Симптом (данные с VPS):** через ~3ч после деплоя v0.40 — 20 полных
анализов (8 scheduled + 12 event), **100% HOLD**. В каждом решении бот
писал «best score 1/5 или 2/5», до порога 3/5 не дошёл **ни разу**.
Источник: `docker logs fx-pro-bot-ai-trader-1` (период 12:55–15:52 UTC),
выборка 20 решений — это не P&L-вывод (sample-size.mdc неприменим), а
диагностика недостижимого порога = баг логики.

**Причина (арифметическая ловушка):** для входа нужно ≥3 из 5 правил MFP.
Правила 1&4 (momentum, breakout = TREND) и 2&3 (BB-Z, RSI = MEAN-REVERT)
взаимоисключающи по направлению → реально набирается максимум **2
ценовых правила**. Третьим обязан быть rule 5 (MACRO REGIME). Но текст
правила требовал НЕнейтрального макро («flat/neutral does NOT fire»), а
крипто-макро (DXY/BTC.D/total cap) почти всегда нейтрален (за 3ч —
`regime neutral` в каждом решении). Итог: 2 (цена) + 0 (нейтр. макро) =
**потолок 2/5 < 3/5** → вечный HOLD. Промпт противоречил сам себе:
docstring обещал «trend = momentum+breakout+macro = 3/5», а текст правила
гасил macro при нейтрале.

**Фикс:** rule 5 теперь **ФИРИТ (+1) при нейтральном ИЛИ поддерживающем**
режиме («neutral counts» — спокойный 3-й голос «режим не против меня»);
**0 только при встречном** режиме; **STRONG opposing** дополнительно даёт
veto (как и раньше). **Порог 3/5 НЕ менялся.** Теперь чистый тренд
(momentum+breakout+macro) и чистый mean-revert (bb-z+rsi+macro) набирают
3/5 — ровно как задумано в docstring. Класс правки: исправление явной
логики / inverted condition (no-data-fitting.mdc «Допустимые правки БЕЗ
нового анализа: bug-fix»), не подгонка порогов под результат.

**Файлы:** `src/ai_trader/llm/prompts.py` (rule 5 текст + REGIME VETO блок
+ HOLD-пример + два «MFP 3/4»→«3/5» + docstring v0.40.1),
`tests/test_ai_trader.py` (SHA baseline `43ec80ff…` → `1233a334…` +
история). Тесты: 208 passed. Триггерит reset эксперимента n=0.
