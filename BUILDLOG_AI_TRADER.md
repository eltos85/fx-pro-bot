...
## 2026-05-08 — feat(prompt v0.8.1): rich-format открытых позиций + анти-путаницы guard

`<hash-pending>`

### Контекст

Пользователь обратил внимание что когда у бота 3+ открытых позиций
одновременно, есть «ощущение что агент их путает». Анализ логов
показал что объективно модель **не путает** — в commentary она верно
перечисляет позиции по символам и реагирует на изменение их числа
(когда AVAX закрылся биржей по SL, в next цикле она написала
«both open positions WLD, TAO» вместо «all 3»).

Но **формат подачи позиций в LLM-контекст был минимальным** и создавал
объективный cognitive load:

```
id=18 Sell WLDUSDT qty=99 entry=$1.234 sl=$1.30 tp=$1.05 lev=2x linkid=ai_xxx
```

В строке нет:
- текущей цены символа,
- unrealised PnL в долларах и в R-units,
- сколько часов позиция открыта (age),
- conviction-уровня (которым открывали),
- оригинального reason (зачем открывали).

Из-за этого правила EXIT MANAGEMENT v0.6 (которые формулируются в
R-units: «unrealised >= 1.5R», «mean-reversion вернулась к VWAP»)
требуют от модели **самой высчитывать** PnL по N позициям параллельно,
держа в голове entry/SL/TP и текущий тикер. На 3+ позициях это объективно
повышает шанс ошибки.

### Решение (cog-load reduction)

В `format_context_for_prompt` (через новый helper `_format_open_position`)
добавлены runtime-метрики для каждой позиции:

```
id=42 Sell WLDUSDT qty=100 entry=$1.5 SL=$1.6 TP=$1.3 lev=2x
    now=$1.4 unrealised=$+10.00 (+1.00R of 2.0R target, +50%→TP)
    conv=medium opened=2h 15m ago
    reason: "WLD stretched, retail long, contrarian short"
```

Расчёты:
- `unrealised` = `(now - entry) * qty * sign` (sign=+1 Buy / -1 Sell).
- `R-units` = `(now - entry) * sign / |entry - SL|` — стандартный риск-нормализованный PnL.
- `target_R` = `|TP - entry| / |entry - SL|` — спроектированное R:R.
- `%→TP` = прогресс по линейной шкале от entry к TP (100% = TP hit).
- `age` парсится из `opened_at` (ISO timestamp в `AiPosition`).
- `conv` извлекается из `llm_reason` префиксом `[conv:X]` (см. v0.8).

Graceful degradation: если current_price=None (тикер недоступен) —
runtime-секция полностью пропускается, остаются базовые поля. Если
SL/TP=None — выводится `?R` (R-units не определены). Если префикса
`[conv:X]` нет — старая позиция, conviction просто не показывается.

### Анти-путаницы guard в промпте

В описании секции `H) OPEN POSITIONS` промпта добавлено явное правило:

> When 3+ positions are open, ALWAYS reference each by `id` AND
> `symbol` together in your commentary (e.g. "id=18 WLD short");
> never close by symbol alone. The JSON `position_id` MUST match the
> `id=` you cited in commentary — this prevents accidentally closing
> the wrong position.

Это форсирует модель явно повторить id+symbol в reasoning, что:
1. Делает чище commentary для пользователя (читаемые upd-сообщения).
2. Снижает шанс что модель в JSON напишет position_id одной позиции,
   а в reason — про другую (классический LLM-mismatch).

### Тесты

В `TestFormatOpenPosition` добавлено 6 unit-тестов:

- `test_short_in_profit_shows_correct_r_units` — Sell @1.50 SL=1.60
  TP=1.30, current=1.40 → +1.0R of 2.0R, +50%→TP. ✓
- `test_long_at_loss_shows_negative_r` — Buy @100 SL=90 TP=120,
  current=95 → -0.5R, -25%→TP. ✓
- `test_no_current_price_skips_runtime_section` — graceful при
  отсутствующем тикере: только базовые поля.
- `test_no_conviction_prefix_back_compat` — старые позиции без
  `[conv:X]` — `conv=` не показывается, остальное работает.
- `test_long_reason_truncated` — reason обрезается на 140 chars + `…`.
- `test_zero_sl_tp_does_not_crash` — `?R` метка вместо краша.

Все 540 тестов прошли.

### Не меняем торговую логику

Это **cognitive-load reduction**, не правка стратегии:
- Те же входные данные (entry/SL/TP/qty/current_price) — модель их и
  так получала по отдельности (тикер в секции D, позиции в H).
- Просто перенесены готовые вычисления PnL/R/% на сторону кода.
- Никакие thresholds, фильтры, R:R-требования не изменились.
- По правилу `no-data-fitting.mdc` — допустимо без OOS-валидации.

### Файлы

- `src/ai_trader/trading/context.py` (новый `_format_open_position`)
- `src/ai_trader/llm/prompts.py` (v0.8.1 docstring + анти-путаницы guard
  в секции H, описание новых полей runtime-метрик)
- `tests/test_ai_trader.py` (`TestFormatOpenPosition`, 6 тестов)
