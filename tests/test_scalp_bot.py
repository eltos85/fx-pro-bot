...
def test_learned_fee_rate_does_not_change_signal_geometry(tmp_path):
    """Замок на решение, закрытое отрицательно (v0.18.53 → BUILDLOG 2026-08-03).

    Выученная ставка — ТОЛЬКО телеметрия. Буквальная подстановка её в гейты
    расширила бы стоп market-стратегиям с 0.30% до 0.44% цены, а это ветка
    между ``x1.5`` и ``x2`` эксперимента ``sl_widen``. Эксперимент закрыт:
    ΔnetR −0.007 [−0.211; +0.197] и +0.081 [−0.139; +0.301] — комиссия в R
    падает втрое, но валовой результат ухудшается настолько же, то есть
    расширение стопа меняет единицу измерения, а не деньги.

    Замер 31.08 (`scripts/collect_scalp_stats.py`) заново подтвердил сам
    разрыв ставок (market-страты платят 0.349–0.368R против 0.227R у
    лимитных), но вывод от этого не меняется: разрыв реален, а починка
    геометрии по нему уже измерена и ничего не даёт. Геометрия обязана
    зависеть только от ``cfg.round_trip_fee_frac``.
    """
...
def test_canon_strategy_in_registry_and_cooldown_family():
    ...
    # v0.18.65: канон ВЕРНУЛСЯ в дефолтный список. Отключение v0.18.59
    # опиралось в том числе на довод «валовой минус», снятый как артефакт
    # перекошенного покрытия fees_usd (1a90ad2): по единой модели ставки его
    # валовой +0.086R выше остальных стратегий. Возвращён ради разнообразия
    # символов — он единственный торгует мейджоров.
    assert "sweep_fade_canon" in s.strategy_list
