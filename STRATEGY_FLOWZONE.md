...
6. **Наблюдаемость (не гейт, 2026-08-17):** heartbeat раз в минуту пишет
   воронку `skip=no_trend:N,shape:N,no_zone:N,away:N,...` и `sess=london|overlap|ny`.
   На сделке в `reasons`: `inst=` (мгновенный classify до латча), `merge=N`
   (сколько прошлых сессий влилось, C1), `sess=`. MAE/MFE в горячий путь не
   добавляем — считать офлайн по принтам.
