...
### fix(deploy): runtime-состояние выведено из-под git + backup/restore в деплое

**Симптом.** `data/ctrader_tokens.json` отслеживался git, но перезаписывается
сервисом в рантайме, поэтому версия в коммите всегда устаревшая (здесь — от
09.04.2026). `git reset --hard origin/main` в `scripts/deploy-on-vps.sh`
откатывал файл, а следующий за ним `docker compose down/up` поднимал
`ctrader-token-service`, который читает диск при старте: refresh просроченным
токеном вернул бы отказ и все cTrader-боты остались бы без авторизации. Три
месяца не проявлялось только потому, что шли селективные rebuild'ы.

**Решение.** Файл выведен из индекса и добавлен в `.gitignore`. Переход
tracked→untracked удаляет файл один раз, поэтому в скрипт добавлены backup до
`reset` и restore после — каталог вне рабочего дерева (`FX_RUNTIME_STATE_DIR`,
по умолчанию `/root/.fx-pro-bot-state`, 0700, файлы 0600). Restore срабатывает
только если файла нет или он пуст, живой токен копией не затирается. Проверено
симуляцией с заглушками `git`/`docker` для обоих случаев. Заодно `docker logs
fx-pro-bot-advisor-1` больше не роняет скрипт под `set -e` (профиль `disabled`).
