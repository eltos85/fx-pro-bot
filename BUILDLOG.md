...
### feat(momentum): запрет новых входов всю пятницу UTC
`<pending commit>`

Решение пользователя после forensics 3y (запись ниже): единственная группа,
отрицательная в обеих половинах истории, — входы в пятницу: −0.081 / −0.101R
net, n=599, p=0.004 (`scripts/_momentum_forensics_3y.py`). Порог Бонферрони
для 26 групп (p<0.0019) не пройден — пользователь принял правку осознанно.
Ожидаемый эффект на current-config 2y: −0.065 → −0.049R/сделку
(`scripts/_momentum_current_config_validation.py`, вариант fri_noentry,
n 1030 → 823). Стратегию в плюс не выводит — уменьшает убыток. Live-пример:
вход Пт 18:04 закрыт friday-flat в 20:03 (9 пятничных входов 08-26→09-25,
−$13.58).

Новый параметр `friday_entry_block_start` (default "00:00",
`MOMENTUM_BOT_FRIDAY_ENTRY_BLOCK_START`); начало блока = min(block_start,
flat_start) — дыра 20:45–21:00 (07-02) не возвращается. Откат: env "20:00".

**Тесты:** +3 (`test_friday_entry_blocked_whole_friday_with_block_start`,
`..._never_later_than_flat_start`, `..._default_whole_day`), сьют 1435 passed.

**Файлы:** `src/fx_momentum_bot/strategy/friday_flat.py`,
`src/fx_momentum_bot/config/settings.py`, `src/fx_momentum_bot/app/main.py`,
`tests/test_fx_momentum_bot.py`, `STRATEGIES.md`

### research(result): собственное исследование — ни одна из 3 гипотез не прошла этап поиска
`<not committed>`

Прогон `scripts/_own_research_3h.py` (правила — запись pre-registration ниже),
вывод `/tmp/own_research_discovery.txt`. Период поиска 2023-09-21→2025-04-30,
порог p < 0.0167. Результат — net в единицах дневного ATR на сделку.

| Гипотеза | n | gross | net | p | Вердикт |
|---|---|---|---|---|---|
| H1 USD-bloc dispersion | 263 | +0.079 | +0.048 | 0.33 | не прошла |
| H2 Month-end hedge rebalancing | 76 | −0.060 | −0.085 | 0.034 | не прошла (знак против статьи) |
| H3 Weekend gap fade | 44 | +0.017 | −0.008 | 0.90 | не прошла |

Проверочный период (2025-05→2026-09) по протоколу НЕ открывался. Знаки
сделок перепроверены в коде — ошибки нет. H2 с обратным знаком не
переворачиваем: это post-hoc подгонка (no-data-fitting.mdc); p=0.034 и так
выше порога. H1 до значимости t≈2.4 нужно ≈6× больше сделок (~10 лет данных).

**Вывод:** стратегия с подтверждённым плюсом не найдена.

### research(pre-registration): собственное исследование — 3 гипотезы, правила зафиксированы ДО прогона
`<not committed>`

Запрос: «придумай ни на что не похожую стратегию с подтверждённым плюсом,
не подгоняй». Правила записаны до первого прогона и после не меняются.

**Данные:** M5 cTrader demo 2023-09-21→2026-09-25: EURUSD, GBPUSD, USDJPY,
AUDUSD, XAUUSD (`data/fxpro_klines/*_M5_3y.csv`); дневные индексы yfinance.
**Разбиение:** поиск 2023-09-21→2025-04-30; проверка 2025-05-01→2026-09-25
— открывается один раз, только для гипотез, прошедших поиск.
**Издержки round-trip (пипсы):** EURUSD 1.4, GBPUSD 1.6, AUDUSD 1.6,
USDJPY 1.8 (спред 0.5 + комиссия $70/1M + 0.2 слиппедж), XAUUSD 7.0;
своп по снимку ProtoOASymbol при переносе через 17:00 NY. Вход — open
следующего M5-бара после сигнала. Результат — в единицах дневного ATR(14).
**Критерий:** поиск — net mean > 0 и p < 0.0167 (0.05/3, Бонферрони);
проверка — net mean > 0 и p < 0.05. Иначе — «не подтверждено».

- **H1 USD-bloc dispersion.** Каждый час 07–20 UTC: 24h-доходности пар в
  знаке «сила доллара» (EURUSD/GBPUSD/AUDUSD с минусом, USDJPY с плюсом);
  фактор = среднее; остаток пары = её доходность − фактор; z = остаток /
  std остатка за 30 дней. |z| ≥ 2 (2σ, Bollinger) → сделка против остатка;
  выход при смене знака z или через 24 ч. Одна позиция на пару.
- **H2 Month-end hedge rebalancing** (Melvin & Prins 2015, JIMF). Последний
  рабочий день месяца: вход 14:00 London, выход 16:05 London (после WM-фикса).
  MTD до вчерашнего close: S&P500 vs местный индекс (EUR ^STOXX50E, GBP ^FTSE,
  JPY ^N225, AUD ^AXJO). S&P сильнее → long иностранная валюта (продажа USD),
  слабее → short.
- **H3 Weekend gap fade** (4 пары + XAUUSD). Гэп = open первого бара недели −
  close последнего пятничного. |гэп| ≥ 0.25 дневного ATR → вход в понедельник
  00:00 UTC к пятничному close, если гэп не закрыт; цель — пятничный close,
  стоп — удвоение гэпа, выход не позже 12:00 UTC.

### research(gold_orb): 3 года M5 — edge живёт только в нереалистичной цене входа (observation only)
`<not committed>`

Проверка единственного кандидата с плюсом в истории проекта (gold_orb:
365d PF 1.57 n=493; live 04-07→05-22 n=83 +185p). Код советника не менялся.

**Данные:** XAUUSD M5 с demo cTrader, 213 168 баров 2023-09-21→2026-09-25
(`data/fxpro_klines/GC_F_M5_3y.csv`, gitignored). Период A 2023-09→2025-04
в подборе параметров не участвовал.

**Скрипт:** `scripts/_gold_orb_3y_validation.py`, вывод `/tmp/gold_orb_3y_out.txt`.
Логика baseline из `backtest_gold_orb_h1_h5.py`; добавлены комиссия
(35 USD/1M за сторону, 2.3p) и своп через 17:00 NY (long −69.25 / short
+17.15 пипсов цены, среда ×3), три варианта цены входа.

| Вход, 1 сделка/сессию | n | avgR net | PF | A (не видела) | B (подбор) | C (после запуска) |
|---|---|---|---|---|---|---|
| ровно граница коробки (старый бэктест) | 1507 | +0.102 | 1.34 | +0.020 (t 0.4) | +0.176 | +0.237 |
| close сигнального бара | 1507 | −0.128 | 0.97 | −0.191 (t −3.9) | −0.093 | +0.019 |
| open следующего бара | 1507 | −0.127 | 0.97 | −0.191 | −0.088 | +0.019 |

Live входит рыночным ордером после касания (`process_signals`,
entry_price=price) — в среднем на 12.4p (0.26R) дальше границы. Весь gross
edge (+0.265R → +0.034R) сидит в этом разрыве. Даже при идеальном входе по
границе период A ≈ 0 (t 0.4). Окно live 04-23→05-22 в модели (close) ≈ 0
(n=39, −123p) — согласуется с live +185p на 83 сделках (шум).

**Найден баг бэктеста multi-entry:** multi × вход по границе даёт WR 77%,
PF 6.9 — артефакт: после выхода следующий бар всё ещё за коробкой, а вход
засчитывается по границе, далеко от рынка. С реалистичным входом multi —
PF 0.83 (t −9). Этим же объясняются цифры в docstring `gold_orb.py`
(«multi-entry 90d WR 75%, PF 6.76»), на которых основан выбор multi-entry
для live. Код не правился (советник выключен с 05-22).

**Вердикт quant-math:** эффект не держится вне периода подбора.

**Файлы:** `scripts/_gold_orb_3y_validation.py` (research-only)

### research(momentum): forensics 3232 сигналов за 730d — разделяющего признака нет (observation only)
`<not committed>`

По запросу «примени аналитика к нашим данным и 3-летней истории»
(trade-forensics → forex-trader → quant-math). Код стратегии не менялся.

**Метод (scripts/_momentum_forensics_3y.py, /tmp/mom_forensics_3y_out.txt):**
yfinance 1h 2023-12-12→2026-09-23, 4 пары, входы базового сигнала без
фильтров, выход — live-механика. IS/OOS split 2025-05-03, терцили на IS.
Сверка с live на том же окне 08-26→09-25: модель +0.073R/сделку gross,
live +0.066R (n=54) — движок воспроизводит реальность.

**Разложение:** n=3232, gross +0.022R (t=1.21), net −0.014R. WR 35.1% при
безубыточном 35.9% с издержками. Полные стопы (903): медиана MFE 0.26R,
лишь 1% доходил до +1R → убыток от качества сигнала, не от управления.
Decay-выходы −0.35R в среднем (режут убыток раньше стопа).

**Форвардный ход сигнала (ATR):** +6ч +0.08 (IS t=1.0, OOS t=1.9) —
≈0.03R, ниже издержек; 12–48ч ≈0 / отрицательно.

**Признаки (10 проверок, Бонферрони p<0.005):** ADX, сила сигнала z24,
волатильность (перцентиль внутри символа), EMA200, положение в 24h-диапазоне,
сторона, символ, D1-тренд, день недели, сессия — ни один не повторяется
IS→OOS. Фильтры 07-24 не подтверждены: низкий ADX в OOS лучший терциль
(+0.083), азиатские часы 00-07 OOS +0.066. Единичные OOS-всплески
(USDJPY t=2.76, вторник t=2.15) при ≈0 на IS — шум множественных сравнений.
Пятница отрицательна в обеих половинах (−0.045 / −0.065, объединённо p≈0.08)
— ниже порога.

**Вердикт quant-math:** эффект не держится — ни одна находка не
воспроизводится на OOS. Фильтрами прибыльное подмножество не выделяется.

**Файлы:** `scripts/_momentum_forensics_3y.py` (research-only)

### research(momentum-pivot): замена стратегии — кандидаты не переживают своп / издержки (observation only)
`<not committed>`

Пользователь выбрал pivot на daily multi-asset TSMOM (кандидат C, запись
07-10). Перед дизайном — проверка допущений + поиск в литературе.

**1. Своп FxPro (ProtoOASymbol с demo, 2026-09-25), % годовых long/short:**
EURUSD −2.9/+0.6, GBPUSD −1.2/−0.9, USDJPY +1.6/−6.5, AUDUSD −1.0/−1.5,
XAUUSD −5.9/+1.5, BRENT +12.5/−57, NAT.GAS −135/+33 (единицы по пипсам —
проверить вручную), BITCOIN/ETHEREUM −20/−20 (PERCENTAGE). #US500_Z26 и
#USOil_V26 — датированные CFD без свопа (USOil tradingMode=1, закрыт).
Кандидат C 07-10 своп не учитывал.

**2. TSMOM со свопом (scripts/_tsmom_cfd_swap_check.py, 10y 1d,
/tmp/tsmom_swap_out.txt):** 10 инструментов 12м Sharpe 0.65 → **0.12**
(OOS −0.30); без крипты 0.56 → −0.07; 3м/6м — все отрицательные. Единственный
положительный срез (6 инструментов, 12м, 0.32, t≈1.0) — выбор лучшего из 9,
не значим. Совпадает с github.com/ilahuerta-IA/carver-systematic-trading
(EWMAC на CFD 26 лет: Sharpe +0.31 → −0.46 со свопом) и Huang/Li/Wang/Zhou
2020 JFE «Time series momentum: is it there?» (TSM ≈ эффект среднего,
предсказуемость слабая). **Кандидат C отклонён.**

**3. Intraday FX time-of-day (Breedon & Ranaldo 2013 JMCB, без свопа;
scripts/_fx_time_of_day_check.py, /tmp/fx_tod_out.txt):** окна из статьи
(Европа 07:00 CET → США 08:00 NY → 16:00 NY), 2023-12→2026-09 — чистый
post-publication OOS. EURUSD gross +0.08 / +0.05 pip на ногу (t≈0) — эффект
1997–2007 исчез; net −2.3 pip/день. **Отклонён.**

**4. Intraday momentum S&P 500 (Zarattini/Aziz/Barbon 2024):** сами не
проверяли (нужны 1-мин данные за годы, yfinance даёт 7 дней). Независимая
репликация (github giovannibrusco/zarattini-2024-momentum-spy): Sharpe 1.11
до 2025, **с 2025 ≈ 0**. Не рекомендован без собственной проверки.

**Вывод:** ни один проверенный кандидат не даёт edge на CFD FxPro после
свопа/издержек. Решение — за пользователем.

**Файлы:** `scripts/_tsmom_cfd_swap_check.py`, `scripts/_fx_time_of_day_check.py`
(research-only)

### research(momentum): текущая конфигурация на 2 годах — edge отрицательный, фильтры 07-24 не помогают (observation only)
`<not committed>`

Запрос «что улучшить для прибыли». Код стратегии не менялся.

**Live (cTrader API, счёт после 26.08, 54 позиции 08-26→09-25):** net +$66.46,
gross +$88.22, комиссия −18.23, своп −3.53 (0.036R/сделку). В R: +0.066 gross,
+0.027 net, sd 1.0 → 95% CI ±0.27R — от нуля не отличимо. Весь плюс дают 12
раннеров (+$223) против 16 полных стопов (−$162); остальные 26 выходов ≈ +$6.
Шорты +$124 (WR 55%), лонги −$58 (WR 28%) — режим укрепления USD, 4 недели.
Сайзинг: 69% сделок упираются в cap 0.05 лота, средний риск $11.1 вместо $15.
Partial фактически 0.4 вместо 0.5 на объёмах 500k (floor по step).

**Бэктест (scripts/_momentum_current_config_validation.py, yfinance 1h
2023-12-11→2026-09-25, 4 пары, вывод /tmp/mom_cur_cfg_out.txt):**

| Вариант | n | avgR (cost 0.036) | 95% CI | IS / OOS netR |
|---|---|---|---|---|
| база июля (без фильтров) | 3632 | −0.021 | ±0.031 | −49 / −28 |
| **текущая конфигурация** | 1030 | **−0.065** | ±0.061 | −33 / −34 |
| без гистерезиса | 1072 | −0.050 | ±0.053 | −24 / −30 |
| без NY-open block | 1399 | −0.033 | ±0.050 | −32 / −14 |
| без ADX≥20 | 2006 | −0.018 | ±0.046 | +15 / −50 |
| без friday-flat | 1048 | −0.082 | ±0.062 | −42 / −44 |
| без входов в пятницу | 823 | −0.049 | ±0.070 | −20 / −20 |
| partial 0.4 | 1030 | −0.062 | ±0.062 | −33 / −32 |

Текущая конфигурация: gross −0.029R, 10 из 12 кварталов в минусе, p≈0.04.
Ни один фильтр 07-24 (ADX, NY-block, гистерезис) на OOS не улучшает
результат — снятие каждого даёт меньший минус (в пределах шума). Friday-flat
единственный компонент с положительным вкладом. Движок сверен: база июля
воспроизводит запись 07-10 (+0.015R gross). Не моделируется: news-close,
лимит 3 позиции.

**Вывод:** live-плюс совместим с нулевым/отрицательным edge (удача + режим
USD). Правки выхода, сайзинга, пятничных входов и partial не делают
стратегию прибыльной. Решение (стоп / pivot на daily multi-asset TSMOM из
записи 07-10 / продолжить демо) — за пользователем.

**Файлы:** `scripts/_momentum_current_config_validation.py` (research-only)
...
**Симптом.** При синхронизации ветки на VPS (`git reset --hard origin/...`)
живой файл токенов был перезаписан версией из коммита `daac72e` (09.04.2026) с
`expires_at ≈ 09.05.2026` — токен трёхмесячной давности. Файл **отслеживается
git**, а сервис постоянно пишет в него свежий токен, поэтому в рабочем дереве он
всегда «modified» и любой hard-reset его откатывает.

**Восстановление.** `TokenManager` читает диск только в `__post_init__`
(`src/ctrader_token_service/manager.py`), поэтому у долгоживущего контейнера
актуальное состояние остаётся в памяти. Диск приведён в соответствие с памятью
средствами самого сервиса (без ручной правки файла), сверка пройдена; срок
действия — до начала сентября. Сделана копия вне git-дерева, доступ 0600.

**Скрытая мина, шире инцидента.** `scripts/deploy-on-vps.sh` делает ровно
`git reset --hard origin/main` **и** `docker compose down/up`. То есть полный
деплой через GH Actions: (1) откатывает токен на апрельский, (2) рестартует
`ctrader-token-service`, который читает диск при старте. Уцелевшей копии в
памяти после рестарта нет, а refresh с трёхмесячным `refresh_token` вернёт
Access denied → все cTrader-боты остаются без авторизации. Пока не сработало
только потому, что 3 месяца шли селективные rebuild'ы (`deploy-vps.mdc`), а не
полный деплой.

**Фикс (решение пользователя — чинить сразу).** Файл выведен из-под git
(`git rm --cached` + правило в `.gitignore`), поэтому `reset --hard` его больше
не касается. Переход tracked→untracked опасен ровно один раз: git удаляет файл,
исчезнувший из HEAD, — поэтому в `deploy-on-vps.sh` добавлены шаги
**backup до reset** и **restore после reset** из каталога вне рабочего дерева
(`FX_RUNTIME_STATE_DIR`, по умолчанию `/root/.fx-pro-bot-state`, 0700, файлы
0600). Restore срабатывает только если файла нет или он пуст, то есть свежий
токен на диске копией не затирается.

Проверено на симуляции с заглушками `git`/`docker`: (1) reset удаляет файл →
восстановлен с правами 0600 и исходным содержимым; (2) файл на месте →
остаётся живая версия, копия обновляется. Заодно `docker logs
fx-pro-bot-advisor-1` больше не роняет скрипт под `set -e` (advisor в профиле
`disabled`).

Замечание на будущее: скрипт всегда сбрасывает VPS на `origin/main`, а сейчас
рабочее дерево на VPS стоит на ветке `feat/ai-trader-v0.30-institutional` —
полный деплой уведёт его на main.

**Файлы:** `scripts/deploy-on-vps.sh`, `.gitignore`,
`data/ctrader_tokens.json` (выведен из индекса)
...
Operational-изменение на VPS (без правки кода): по просьбе пользователя порог
активации страховки поднят с $2 до $5 — `MOMENTUM_BOT_PROFIT_PROTECT_ACTIVATE_USD`
в `.env` 2.0 → 5.0, `--force-recreate` контейнера. Защита 80% теперь включается
при gross ≥ $5 (gross $5 → защищает $4; $6 → $4.8; $10 → $8). Позициям даётся
больше пространства до включения защиты, малые прибыли ($2–5) не режутся.

**Файлы:** `.env` (VPS) — код не менялся.
...
Продолжение аудита канона (запись ниже). Пользователь решил менять
стратегию (пункты 2 и 3 из аудита). Research ДО кода
(`scripts/_pivot_candidates_research.py`, no-data-fitting):

**Кандидат B — intraday mean-reversion FX-мажоров (пункт 2): НЕ ПРОШЁЛ.**
Канонические параметры без тюнинга (BB(20,2σ) Bollinger 2001; RSI(14)
30/70 Wilder 1978; session [7,21); guard capitulation 4×ATR Chande/Kroll;
SL 2.5×ATR; издержки −0.06R/сделку — live-замер):
- B1 (BB fade): n=2434 за 2 года, WR 58%, но **netR −159, PF 0.82** —
  все 6 полугодий в минусе. Gross ≈ −13R: сам edge ~0, издержки добивают.
- B2 (BB+RSI): n=466, netR −35, PF 0.81.
Совпадает с Neely/Weller 2003: intraday-FX паттерны не переживают costs
на розничном исполнении. Пункт 2 закрыт данными.

**Кандидат C — канонический multi-asset daily TSMOM (пункт 3): ПРОШЁЛ.**
Moskowitz/Ooi/Pedersen 2012: sign(momentum), vol-scaling 40%, месячный
ребаланс; вселенная — 10 инструментов, доступных на cTrader-счёте
(4 FX-мажора, XAUUSD, CL, NG, US500, BTC, ETH), 10 лет, 5bp/turnover:

| Lookback | annRet net | Sharpe | maxDD |
|---|---|---|---|
| 3м | +6.3% | 0.40 | −29% |
| 6м | +9.1% | 0.50 | −41% |
| **12м** | **+14.5%** | **0.88** | −29% |
| combo 3/6/12 (Hurst 2017) | +9.4% | 0.63 | −28% |

Честные оговорки: (1) вклад BTC доминирует (+118% из кумулятива —
захватил бычий 2017); без крипты 12м Sharpe 0.39 — совпадает с
литературой для узких вселенных, но скромнее; (2) 2026 YTD −7% (TSMOM
имеет многомесячные просадки — канон, а не дефект); (3) 5bp — оценка,
реальные cTrader-спреды крипты выше (проверить при имплементации);
(4) сделок мало (~10-30/мес ребаланс-легов) — sample-size копится
медленно, это свойство канона.

**Вывод:** разворот бота на daily multi-asset TSMOM (12м или combo),
дизайн имплементации (вселенная, ребаланс, sizing/leverage, SL-политика)
— на согласовании с пользователем. Текущий 1h-momentum рекомендован к
отключению (edge отрицательный после издержек).

### research(momentum): валидация канона — 24h-TSMOM на FX-мажорах не имеет edge

Аудит канона по запросу пользователя («верен ли канон вообще»). Наблюдение
без изменения кода (sample-size.mdc: фиксируем в BUILDLOG, решение — с
пользователем).

**Заявленный канон vs литература.** Ядро бота — 24-барный 1h momentum,
порог 0.15% — цитирует Moskowitz/Ooi/Pedersen 2012 (TSMOM). Но канонический
TSMOM: **12-месячный** lookback, **58 диверсифицированных фьючерсов**,
месячный ребаланс, vol-scaling. Перенос на 24 часа × 4 FX-мажора канон НЕ
поддерживает:
- Menkhoff/Sarno/Schmeling/Schrimpf 2012 (JFE): FX momentum живёт на
  formation 1–12 **месяцев**, прибыль сконцентрирована в минорах с широким
  спредом; спред съедает ~50% returns.
- Neely/Weller 2003 (FRB St. Louis): intraday FX technical rules → нет
  excess returns после реалистичных издержек.
- arXiv 2501.16772 (2025): на масштабе часов только слабые тренды
  персистентны; intraday-FX умеренные ходы — отрицательная автокорреляция
  (mean reversion), quantile-regression evidence.

**Собственные тесты (scripts/_momentum_canon_validation.py, yfinance 1h
730d + 1d 10y, 4 пары):**

| Тест | Результат |
|---|---|
| Реплика стратегии, 2 года 1h, gross (mid) | n=3655, **+0.02R/сделку**, PF 1.06 — edge неотличим от нуля, по кварталам ±50R |
| То же с live-издержками −0.06R/сделку (замер BUILDLOG 2026-07-10) | **−135R** за 2 года, PF 0.91, IS и OOS оба в минусе |
| Vol-нормированный порог (замена фикс 0.15%) | не спасает: −193R, PF 0.87 |
| Форвардный ход после флипа сигнала (без exit-механики) | +0.085 ATR @6ч (p=0.053), ~0 @12ч, **отрицательный** @24–48ч — сигнал не предсказывает движение на горизонте удержания |
| Канонический дневной TSMOM sign-rule на этих же 4 парах, 10 лет | Sharpe −0.13…+0.11, все p>0.4 — на мажорах TSMOM не работает и на канонических горизонтах |

**Ревизия вывода от 2026-07-10 (диагностика implementation gap):**
5m-реплика на live-окне (+35R) оказалась чувствительна к гранулярности —
1h-движок на том же окне даёт ~0R gross. Дубли-входы (−10.1R) и стоимость
исполнения (−7.3R) реальны (broker-truth) и фикс per-symbol гарда верен,
но базовый edge стратегии ≈ 0: живой минус — это **canon gap + издержки**,
не только исполнение. Матожидание при gross 0.02R и cost 0.06R ≈
−0.04R/сделку × ~5 сделок/день — согласуется с live −0.16R (хуже из-за
дублей).

**Вывод:** дыра не в реализации, а в основании — 24h-momentum на 4
FX-мажорах не подтверждается ни литературой, ни 2-летним бэктестом, ни
10-летним дневным тестом. «Закрыть дыру» тюнингом порога/фильтров нельзя
(vol-norm проверен — нет). Честные опции: (а) остановить/paper-mode;
(б) сменить класс стратегии на поддержанный литературой для мажоров
(session-based, mean-reversion умеренных ходов); (в) сменить вселенную
(диверсифицированный multi-asset TSMOM на дневках — но это другой бот).
Решение за пользователем.

**Артефакты:** `scripts/_momentum_canon_validation.py`,
live-выборка 122 сделки (`data/loss_audit_trades_0710.json`), терцили
live: слабый |m| avgR −0.26 vs сильный −0.06 (шум у порога).

### fix(momentum): per-symbol гард входа — стоп дублям позиций + диагностика live-vs-backtest

**Диагностика разрыва live/бэктест** (по запросу «диагностику делай»):
реплика стратегии (BE@1R, partial@1.5R, trailing, sign-decay,
`scripts/momentum_exit_backtest.py`) прогнана на live-окне
2026-06-05→07-10, 4 FX-пары, с учётом дат деплоя session-фильтра (06-26)
и friday-block (07-02):

| | n | netR | WR | PF |
|---|---|---|---|---|
| Реплика (mid, live-фильтры) | 93 | **+35.1R** | 48% | 2.48 |
| Live (broker-truth) | 122 | **−19.4R** | 30% | 0.63 |

Рынок для стратегии был хорошим (реплика в плюсе 5 из 6 недель) →
**implementation gap**, не режим. Декомпозиция (пер-сделочный матчинг
live↔реплика, символ+сторона, вход ±2ч):

1. **Дубли-входы: 22 сделки, −10.1R (−$106)** — live открывал 2-ю/3-ю
   позицию той же пары/стороны при живой первой (USDJPY long 06-05
   13:24+14:40+14:55; 06-12 20:01+20:06 и т.д.). Два механизма по
   `momentum_decisions`: (а) дребезг momentum вокруг threshold
   (long→flat→long при открытой позиции — edge-trigger видит «новый
   флип»); (б) retry после отказа slippage-guard, чей аварийный close
   не прошёл — позиция жила на брокере, бот открывал вторую
   (AUDUSD 06-22 00:03→00:08, GBPUSD 07-02 08:02→08:07).
2. **Стоимость входа ≈ −7.3R** — fill в среднем на +0.06R хуже
   сигнального close (спред + до 5 мин задержки от закрытия бара);
   отсюда же гэп на стопах (sl_hit: −10.9R на 14 matched-сделках —
   live-стоп задет спред-крылом, mid-реплика проезжает).
3. **Пропущено прибыльных входов на +24.7R** (41 вход реплики без
   live-пары): session-фильтр отсёк +8.4R (18 входов вне 07–21 UTC —
   на этом окне фильтр в минус себе; по sample-size.mdc НЕ трогаем,
   наблюдение зафиксировано), остальное — state-эффекты edge-trigger
   после рестартов/skip.

**Не подтвердилось:** семантика sign-decay (live zero-cross vs бэктест
full-flip) — A/B на том же окне: zero-cross даже лучше (+50.7R vs
+40.2R); ATR live≈реплика (ratio 0.99); даунтайм 07-04→06 ≈ 0R.

**Фикс (только п.1 — багфикс соответствия бэктест-модели «один трейд
на символ», вход только при `pos is None`):** `_has_same_side_position`
в `wants_open` + note `skip:already_open`; direction фиксируется
(сигнал отработан существующей позицией — эквивалент бэктеста, где
`last_direction` обновляется каждый бар). Противоположную позицию гард
не трогает — её закрывает sign-decay. Пороги/фильтры/exit-логика не
менялись.

**Артефакты:** `scripts/_momentum_live_vs_backtest.py` (реплика на
окне), `scripts/_momentum_gap_match.py` (матчинг + декомпозиция),
`scripts/_momentum_exit_semantics_test.py` (A/B zero-cross/full-flip),
`data/loss_audit_trades_0710.json` (broker-truth). Тесты:
`test_has_same_side_position_*`, сьют 1224 passed.

**Файлы:** `src/fx_momentum_bot/app/main.py`,
`tests/test_fx_momentum_bot.py`, `scripts/_momentum_*.py`
...
cTrader deal-list + контекст входа из `momentum_decisions`, запуск в
контейнере tradecard-momentum read-only), дамп
`data/momentum_loss_audit_trades.json` (103 закрытых сделки).

**Итог периода (06-05→07-02, 103 сделки):** net **−$189.08**, WR 31%,
PF 0.59, avgR −0.14. Сегменты: PRE-baseline (до 06-26, 80 сделок,
−$100.92, PF 0.69) / POST (23 сделки, −$88.16, PF 0.35 — n мал, шум).

**Структурная причина минуса — экспектанси-математика:** avg win $8.66
vs avg loss $6.57 → payoff 1.32 при WR 31%; breakeven для такого payoff
= 43% WR. Система задумана как trend-following 2:1+ (SL 2.5 ATR,
partial@1.5R, trailing), но реализованный payoff 1.32: правый хвост
слишком тонкий для 31% WR.

**Куда уходят деньги (весь период):**
- **SL-хиты:** n=29, −$274 (59% валового лосса), все ровно ~−1R (за SL
  ничего не проскальзывает, гэпов нет — исполнение чистое).
- **Early-exit (sign-decay/флип):** n=35, −$180, средний −0.4R —
  «медленная смерть» до SL.
- **Быстрые смерти:** 18 лоссов с hold <2ч = −$123 — вход на излёте
  движения, разворот сразу после входа.
- **Дребезг edge-trigger:** re-entry в тот же символ ≤6ч: n=31, −$82
  (WR 29%); из них флипы направления ≤6ч: n=10, −$51 — пила в рейндже.
- **Costs (swap+comm):** лишь 4–5% лосса — НЕ проблема.

**По инструментам:** главный минус — **GBPUSD long: n=19, −$100.59**
(7 SL-хитов подряд-кластерами 06-15..23 и 07-02); USDJPY long PRE
−$28 (n=15, WR 20%); EURUSD short POST −$31 (n=3, шум). Единственный
устойчивый плюс PRE — EURUSD short +$35 (n=8).
В −$28.6 GBPUSD 07-02 входит дубль из бага clientMsgId (обе позиции
закрылись по SL 07-02 11:42/11:53; сам баг уже пофикшен, `9804e8e`).

**По времени (PRE, до фильтров):** Asia −$85 (WR 15%, n=20) и Friday
−$60 (WR 13%, n=15) — оба среза УЖЕ закрыты session-filter'ом и
friday-flat от 06-26; Monday −$66 (WR 22%, n=18) — фильтра нет.
POST: NY −$70 (n=12) при London −$19 (n=11) — на n=12 не вывод.

**Сила сигнала не предиктивна:** |mom| медианы лоссов/винов 0.0020 vs
0.0022 (нет разделения — совпадает с tradecard Spearman ρ=0.00);
«погоня за выносом» |mom|≥0.004 даже слегка в плюсе (n=8, +$8).

**Live vs backtest (расхождение — ключевой вопрос):**
`momentum_exit_backtest.py` (06-16, 3 мес, идеальные mid-входы, без
спреда): WR 47%, PF 2.04. Live: WR 31%, PF 0.59. Разрыв WR −16 п.п.
Кандидаты-причины (по fxpro-audit.mdc §3 сначала операционные): спред/
комиссия на «мелких» R (лосс −0.4R легко перевешивает), вход по ask
против mid бэктеста, edge-trigger дребезг в живом потоке (31 re-entry).
Продолжаем forward-сбор; при n≥100 POST — повторный сравнительный
прогон.

**Рекомендации (все — ГИПОТЕЗЫ, n<100 POST, sample-size.mdc: без правок):**
1. Наблюдать GBPUSD long отдельно (главный концентрат лосса, n=19<100 —
   отключать нельзя).
2. Дребезг re-entry ≤6ч (−$82) — кандидат на cooldown-фильтр, но только
   после OOS-бэктеста самого cooldown.
3. Monday-минус PRE — наблюдение, фильтр не предлагаем (n=18).
4. Ничего в порогах/exit-логике не менять: payoff 1.32 может быть
   артефактом рейнджевого месяца — TSMOM живёт трендовыми периодами.

**Compliance:** observation only; изменения стратегии не внесены
(sample-size.mdc, no-data-fitting.mdc); все числа — broker-truth
cTrader deal-list (ctrader-pnl.mdc); окна невалидных данных не
задеты (VP-сделки исключены атрибуцией по вселенной momentum).

**Файлы:** `scripts/momentum_loss_audit.py` (новый, read-only),
`data/momentum_loss_audit_trades.json` (артефакт).
...
**Baseline:** 2026-06-26 08:15 UTC (последние правки логики: friday-flat
`06f3465` + session-filter `51294fa`). Три источника сверены: docker-логи
бота, `momentum_bot.sqlite` (33 080 decisions, 23 executed opens за окно),
cTrader deal-list через свежий прогон `tradecard-momentum weekly`
(broker-truth, ctrader-pnl.mdc).

**P&L (cTrader deal-list, NET):** n=22 закрытых, WR 41%, net **−$40.48**
(gross −$32.23, swap −$0.48, comm −$7.77), EXP −0.13R. По символам:
EURUSD n=5 −$44.75, AUDUSD n=7 −$25.46, GBPUSD n=6 +$5.69, USDJPY n=4
+$24.04. **n=22 << 100 → только наблюдение (sample-size.mdc), стратегию
не трогаем.** Грейд по силе сигнала не монотонен (Spearman ρ=0.00), но
на 20 сделках это шум. Артефакт: `data/tradecard/momentum_2026-27.md`
(volume `tradecard_momentum_data` на VPS).

**БАГ 1 (критичный, исполнение — НЕ стратегия): cross-matching
ExecutionEvent в `client._send_and_wait`.** Симптом (3+ эпизода: 06-26
03:03, 06-29 10:00, 06-30 12:02, 07-02 08:02): сразу после `DECAY CLOSE`
одного символа SLIPPAGE GUARD срабатывает на ДРУГОМ символе с pid
закрытой позиции и абсурдным slippage (16 057 / 6 334 / 4 480 / 1 930
pip — цена одного символа сравнивается с ценой другого), guard-close
падает POSITION_NOT_FOUND. Причина: `_send_and_wait` матчит ответ только
по `payloadType`; каждый ордер/close порождает ≥2 ProtoOAExecutionEvent
(ORDER_ACCEPTED + ORDER_FILLED), «хвостовое» событие съедается waiter'ом
СЛЕДУЮЩЕГО запроса. Доказанные последствия 07-02 08:02: открытия GBPUSD
long и USDJPY short записаны в БД как failed (`live_open:slippage` /
`POSITION_NOT_FOUND`), но реально исполнились на брокере (#152224582,
#152224583 — подхвачены label-сканом `_collect_managed_positions` в
08:07); бот, считая GBPUSD-вход неудачным, повторил его в 08:07:09
(#152224639) → **ДУБЛЬ GBPUSD long, риск ×2 (~$30 вместо $15)**. Фикс —
корреляция запрос↔ответ (clientMsgId cTrader Open API или матч по
orderId/positionId в ExecutionEvent) — требует сверки с официальной
докой (api-docs.mdc), кандидат на отдельный bug-fix коммит (executor +
client — общая инфраструктура, затрагивает и fx_ai_trader).

**БАГ 2 (мелкий, гигиена): дыра пятничного окна + MARKET_CLOSED-спам на
открытиях.** 06-26 (Пт) 21:03–21:59 — 13 попыток открыть EURUSD long
каждые 5 мин, все отвергнуты MARKET_CLOSED. Две составляющие:
(а) friday-flat блокирует входы только в [20:00, 20:45) — после 20:45 и
до закрытия рынка (~21:00+ UTC) входы снова разрешены, а session-filter
проверяет час ЗАКРЫТОГО бара (в 21:03 это час 20 → проходит [7,21));
если бы рынок закрылся чуть позже, позиция уехала бы в выходные вопреки
цели friday-flat. (б) для открытий нет MARKET_CLOSED-дедупа (для decay
close сделан 06-15). Кандидаты: расширить блок входов до конца пятницы;
дедуп-лог на open.

**Ops-состояние на 07-02 11:00 UTC:** на брокере 3 momentum-позиции —
GBPUSD long ×2 (#152224582 entry 1.33406, #152224639 entry 1.33499 —
дубль из БАГа 1) + USDJPY short #152224583. Сопровождение (BE/partial/
trail) работает по всем трём (label-скан). Пользователю решить: закрыть
ли один из дублей GBPUSD вручную.

**Compliance:** изменений стратегии нет; n=22 < 100 → P&L-выводы =
наблюдение (sample-size.mdc); оба бага — технические дефекты исполнения
(допустимая категория no-data-fitting.mdc), фиксы отдельными коммитами
после одобрения; фикс БАГа 1 — только со ссылкой на официальную доку
cTrader (api-docs.mdc).
...
**Контекст:** пользователь заметил, что позиции могут не закрыться до
остановки торгов в пятницу и уехать в выходные. Разбор: для momentum
это канон (Turtle/TSMOM держат через выходные — гэпы в свою сторону
часть трендового edge; факт 06-05→06-08: три USDJPY пережили выходные
штатно, своп +$0.04). Для VP — обоснованная дыра: day-timeframe канон
(Dalton 2007) требует выхода внутри сессии, профиль понедельника —
другой рынок, гэп выходных исполняет SL вне планового 1R.

**Решение:** открытые VP-позиции принудительно закрываются по рынку в
пятницу в окне 16:45–17:30 NY (до CME close 17:00 ET; верхняя граница —
чтобы не спамить close-ордерами в закрытый рынок). Неудавшийся close
повторяется в следующем цикле внутри окна. Momentum-позиции правило НЕ
трогает. Env: `MOMENTUM_BOT_VP_FRIDAY_FLAT_ENABLED` (on),
`..._START`/`..._END` (16:45/17:30, vp_session_tz).

**Тесты:** +5 (`_vp_friday_flat_due`: окно пятницы, рано/поздно, другой
день, off-switch). Сьют 1266 passed.
...
**Симптом (broker-truth, `momentum_pnl_audit.py` 06-05→06-10, 28 сделок):**
VP-шорт золота открыт 12:22 UTC — за 8 минут до US CPI (12:30 UTC).
Релизный спайк ~24 пункта → полный стоп −$24.70 за 8 минут. Ре-вход
13:28 на раздутом пост-релизном ATR (13 vs 8) → −$32.61. Две сделки
одного релизного окна = 90% дневного убытка. Параллельные momentum-стопы
EURUSD/GBPUSD на том же релизе −$4.75.

**Причина:** у momentum/VP-бота не было календаря macro-релизов вообще.
Дыра существовала с рождения стратегии, но была замаскирована
базис-багом slippage-guard (резал 8/9 VP-входов, см. фикс f8e8782) —
после фикса VP впервые реально торговала, и первый полноценный день
пришёлся на CPI-day.

**Анализ «не сломали ли мы прибыльную страту» (запрос пользователя):**
сверка деплоев с broker-сделками показала — НЕТ. (1) VP добавлена
09.06, 11 сделок, «прибыльность» = один выброс +$48.88; (2) до
базис-фикса 8 VP-входов убивались slippage-guard'ом в ту же минуту
(−$3.7 чистого спреда — видно в выписке), фикс это устранил; (3)
momentum FX после правок объективно чище: дриблинг 3×USDJPY исчез,
sign-decay снял +$3.03/+$3.35 по AUDUSD. Откатывать нечего — правки
чинили воспроизводимые баги; сегодняшний убыток — отсутствовавший
фильтр, не сломанный механизм.

**Решение:** `strategy/event_guard.py` — `high_impact_event_near()`,
блок НОВЫХ входов (momentum + VP) в окне ±60 мин вокруг HIGH-impact
событий (US CPI / FOMC / NFP). Календарь переиспользован из
`fx_ai_trader.data.econ_calendar` (shared-инфраструктура: фактические
даты релизов BLS/Fed, не торговый параметр). Сопровождение
(BE/partial/trailing), sign-decay и SL НЕ блокируются — риск-менеджмент
важнее канона входа. Сигнал в окне не теряется: direction не
фиксируется (edge-trigger), попытка повторяется после окна. Сбой
календаря не блокирует торговлю. Env: `MOMENTUM_BOT_NEWS_BLOCK_ENABLED`
(on), `..._BEFORE_MIN`/`..._AFTER_MIN` (60/60).

**Research basis:** Andersen/Bollerslev/Diebold/Vega 2003 (AER) — пик
цены/волатильности в первые минуты макро-анонсов (та же база, что
news-фильтр ±4ч Outsiders); Dalton «Mind Over Markets» 2007 — релиз
сбрасывает аукцион, дорелизный профиль (VA/POC) невалиден. Окно ±60 мин
уже Outsiders'овских ±4ч: breakout/momentum менее чувствительны к
послерелизному тренду, чем mean-reversion. Покрытие календаря: CPI/FOMC
(static 2026) + NFP (правило); PPI/Retail Sales/GDP не покрыты —
известное ограничение.

**Тесты:** +6 в `test_fx_momentum_bot.py` на реальных датах календаря
(CPI 06-10 12:30 UTC: блок в 12:22 и 13:28 — обе фактические сделки
были бы предотвращены; границы окна; FOMC 06-17; тихий день). Сьют
1239 passed.

**Файлы:** `src/fx_momentum_bot/strategy/event_guard.py` (новый),
`src/fx_momentum_bot/app/main.py`, `src/fx_momentum_bot/config/settings.py`,
`tests/test_fx_momentum_bot.py`

### feat(momentum-bot): раздельные лимиты позиций momentum / VP (согласовано)

`<pending>`

**Симптом:** `max_open_positions=3` был общим на обе стратегии: 3 momentum-
позиции (FX) занимали все слоты и блокировали вход VP-стратегии по золоту
(и наоборот). Пользователь: «хочу чтобы было место для других позиций,
иначе 3 позиции одной страты не дадут открыться другой».

**Решение:** лимиты стратегий разделены. `max_open_positions` теперь
считается ТОЛЬКО по momentum-символам (`_count_open_positions_for_symbols`
получает momentum-список без VP). VP гейтится собственными лимитами,
которые и так были: 1 позиция на VP-символ (already_open) + дневной лимит
2/сторону (`vp_max_trades_per_dir_per_day`); проверка глобального
open_count из `_process_vp_symbol` убрана (note `vp_skip:max_positions`
упразднён). Итог: максимум 3 momentum (FX) + 1 VP (GC=F) = 4 позиции,
стратегии не блокируют друг друга. Риск-профиль: +1 позиция 0.01 лота
к худшему случаю (~$3-4 риска), корреляционный аргумент Tharp/Vince не
нарушен — VP торгует золото, не коррелированный с FX-мейджорами класс.

Тесты: сьют 1204 passed (поведенческих тестов на гейтинг VP по глобальному
лимиту не было).
...
**Симптом:** `yfinance: $USDJPY=X: possibly delisted; no price data found
(period=3mo)` по ОДНОМУ тикеру, при том что EURUSD/GBPUSD/AUDUSD в том же
цикле качаются нормально. На VPS USDJPY=X успешно грузился в большинстве
циклов (signal=flat, close≈160), сбой был разовый (2 раза) — транзиентная
флакота Yahoo, не делистинг.

**Поведение до фикса:** при пустых данных `build_signal` → None →
запись `not_enough_data`, цикл по символу пропускается (без краша/ложной
сделки), восстановление на следующем цикле (через 5 мин). Безопасно, но
теряется потенциальный сигнал и шумит ERROR-лог.

**Решение:** `_fetch_candles` получил лёгкий retry (3 попытки, backoff
2s/4s) на пустой результат или исключение. Это сглаживает разовую флакоту
yfinance. Чисто техническое улучшение — пороги/логика стратегии не тронуты.
...
- На брокере 0 открытых позиций **в момент проверки** — остановка
  безопасна. См. corrective ниже про race condition.
...
**Corrective (обнаружено 2026-05-18 12:55 UTC).** Между backup'ом БД
(11:49:56 UTC) и фактическим `docker stop` была пауза ~7 минут, в
течение которой Advisor сделал ОДИН live-цикл и открыл сделку:

- `advisor_stats.positions.id = ef25d270-ae0d-4771-aa6a-a660684dc487`
- `created_at = 2026-05-18T11:56:47 UTC`
- `strategy = gold_orb`, `source = gold_orb_breakout`
- `XAUUSD long 0.04 lots @ $4551.43`, SL=$4540.29
- `broker_position_id = 150835974`, label `fx-pro-bot`

Позиция осталась orphan на брокере с SL/TP (broker сам закроет при
срабатывании). Ни fx-ai-trader (label=`ai-fx-trader`), ни fx-ai-trend
(label=`ai-fx-trend`) её не видят и не трогают — label-isolation
работает корректно.

**Урок процедуры.** Перед `docker stop` всегда делать `reconcile()`
**прямо перед** командой, не за минуты до. Backup БД не блокирует
дальнейшую работу контейнера. Будущий wind-down procedure:
1. Скейлим Advisor в paper-only mode (или `trading_disabled=true`).
2. Ждём `reconcile()` = 0 живых позиций.
3. Только тогда backup + stop + rm.

**Файлы:** `docker-compose.yml`, `BUILDLOG.md`, `BUILDLOG_AI_FX_TRADER.md`,
`scripts/fx_ai_dump_all_positions.py` (diagnostic).
