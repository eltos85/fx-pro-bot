...
**Не исправлено, нужно решение.** Правильный фикс — убрать runtime-файл из git
(`git rm --cached` + `.gitignore`), но переход tracked→untracked означает, что
первый же деплой **удалит** файл на VPS (git чистит файлы, исчезшие из HEAD),
поэтому вместе с этим нужен restore-шаг в `deploy-on-vps.sh` из копии вне
git-дерева. До решения: **не запускать полный деплой** — только селективный
rebuild.

**Файлы:** код не менялся; запись фиксирует операционное ограничение для
