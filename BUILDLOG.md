...
**Симптом:** Пользователь видел открытую позицию XAUUSD, которая в моменте
доходила до +$15 нереализованной прибыли, но не закрывалась — цена откатилась
обратно, и позиция осталась с -$1. На вопрос «почему trailing не сработал»
выяснилась архитектурная проблема:

1. `peak_price` в БД обновляется только по **close** последнего M5 бара.
2. Бот опрашивает раз в `POLL_INTERVAL_SEC=300`, intra-bar пики не видит.
3. `_update_broker_trailing_sl` тащит SL у брокера на основе `peak_price`,
   но если peak не зафиксирован — SL не двигается.

**Backtest сравнение** (`scripts/backtest_gold_orb_trailing_compare.py`,
2 года GC=F M5, 989 сигналов):

| Variant | period | n | net pips | WR | PF | trail % |
|---|---|---|---|---|---|---|
| A NO_TRAIL (baseline) | OOS | 403 | +12 009 | 42.7 | 1.56 | 0 |
| B BOT_LAG_5/3 (как было) | OOS | 403 | +12 053 | 74.2 | 2.73 | 73.7 |
| **C SERVER_RT_5/3 (новое)** | **OOS** | **403** | **+20 832** | **85.4** | **6.53** | **89.6** |

Variant C даёт **+73% pips на OOS** против baseline и +73% против
текущего bot-lag — за счёт обновления peak по high/low бара
(intra-bar peak), а не close.

**Изменения:**

1. `src/fx_pro_bot/strategies/monitor.py`:
   - `PositionMonitor.run()` принимает `recent_bars: dict[str, Bar] | None`.
     Если есть — peak/trough обновляются по `bar.high` (long) / `bar.low`
     (short). Без recent_bars — fallback на close (обратная совместимость).
   - В `_check_exits` для `gold_orb` отключён bot-side `scalp_trail` —
     trailing полностью отдан брокеру через `_update_broker_trailing_sl`.
     Это убирает race condition между closure от monitor и broker SL hit.
2. `src/fx_pro_bot/app/main.py`:
   - В `_run_cycle`: формируем `recent_bars = {sym: bs[-1] for sym, bs in
     bars_map.items() if bs}` и передаём в `monitor.run()`.
   - В блоке scalping/swing extra-bars (GC=F, BZ=F и т.д.) теперь добавляются
     в `bars_map`, чтобы `recent_bars` был полным.
3. `tests/test_strategies.py`: +4 unit-теста (peak по high для long, peak
   по low для short, fallback на close без recent_bars, gold_orb не
   закрывает по scalp_trail).

**Что НЕ изменено:**
- Параметры trailing: `SCALPING_TRAIL_TRIGGER_PIPS=5.0`,
  `SCALPING_TRAIL_DISTANCE_PIPS=3.0` — те же что раньше.
- Логика входа gold_orb (touch-break ORB).
- Static SL/TP при открытии (1.5×ATR / 3.0×ATR).
- Trailing для других scalping-стратегий (vwap, statarb, session_orb) —
  оставлен как был.

**Тесты:** все 330 pytest-тестов проходят (было 310 → +4 monitor + другие
обновления = 314+; +правки в test_outsiders_realism совместимы с новой
сигнатурой run()).

**Файлы:** `src/fx_pro_bot/strategies/monitor.py`,
`src/fx_pro_bot/app/main.py`, `tests/test_strategies.py`,
`scripts/backtest_gold_orb_trailing_compare.py` (новый),
`STRATEGIES.md §3b-bis (раздел trailing)`.
